// -*- C++ -*-
//
// Package:    MargueriteZDCAnalyzer
// Class:      MargueriteZDCAnalyzer
// 
/**\class MargueriteZDCAnalyzer MargueriteZDCAnalyzer.cc Analyzers/MargueriteZDCAnalyzer/src/MargueriteZDCAnalyzer.cc
...
// $Id: MargueriteZDCAnalyzer.cc,v 1.1.1.1 2010/01/21 00:44:19 ssen Exp $
//
//
...
#include "Analyzers/MargueriteZDCAnalyzer/interface/MargueriteZDCAnalyzer.h"
#include "TH1F.h"
#include "TH2F.h"
#include "TStyle.h"
#include "TCanvas.h"
...
MargueriteZDCAnalyzer::MargueriteZDCAnalyzer(const edm::ParameterSet& iConfig){
...
MargueriteZDCAnalyzer::~MargueriteZDCAnalyzer(){
}

void MargueriteZDCAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	++evtNo;
    ...
    std::cout<<"bunchCrossing = "<<iEvent.bunchCrossing()<<std::endl;
...
//	edm::Handle<ZDCDigiCollection> zdc_digi_hc;
//	const Digi *zdc_digi_digi = zdc_digi_hc.failedToGet()? 0 : &*zdc_digi_hc;
       double EM_Pnoise[5];
       double EM_P_EnergyCheck[5];
       double EM_Mnoise[5];
       double EM_M_EnergyCheck[5];
       double HAD_Pnoise[5];
       double HAD_P_EnergyCheck[5];
       double HAD_Mnoise[5];
       double HAD_M_EnergyCheck[5];       
	...
	for (int iz=0;iz<5;iz++){
		...
		DigiEMM[iz]=0.0; 
        EM_Pnoise[iz] =0.0;
        EM_P_EnergyCheck[iz] = 0.0;
        EM_Mnoise[iz] =0.0;
        EM_M_EnergyCheck[iz] = 0.0;
        HAD_Pnoise[iz] =0.0;
        HAD_P_EnergyCheck[iz] = 0.0;
        HAD_Mnoise[iz] =0.0;
        HAD_M_EnergyCheck[iz] = 0.0;
    }
...
//       std::cout<<"ZDCP_EM, going to fill some noise histos"<<std::endl;
                double ZDCP_EMNoise = EM_Pnoise[iChannel-1]*0.025514;
                h_ZDCP_EM_Noise[iChannel-1]->Fill(ZDCP_EMNoise);
                double ZDCP_EMEn = EM_P_EnergyCheck[iChannel-1]*0.025514;
//                h_ZDCP_EM_En[iChannel-1]->Fill(ZDCP_EMEn);
				}
				if (iSide == -1 ){
...
//       std::cout<<"ZDCP_EM, going to fill some noise histos"<<std::endl;
                double ZDCM_EMNoise = EM_Mnoise[iChannel-1]*0.025514;
                h_ZDCM_EM_Noise[iChannel-1]->Fill(ZDCM_EMNoise);
                double ZDCM_EMEn = EM_M_EnergyCheck[iChannel-1]*0.025514;
//                h_ZDCM_EM_En[iChannel-1]->Fill(ZDCM_EMEn);
				}
			 } //0.78283
			 if(iSection==2){
			     	if (iSide ==1 ){
...
//				int chid = (iSection-1)*5+(iSide+1)/2*9+(iChannel-1);
//				std::cout<<"RecHit energy = "<<zhit->energy()<<", Energy from Digi = "<<HAD_P_EnergyCheck[iChannel-1]*0.78283<<", Noise from Digi = "<<HAD_Pnoise[iChannel-1]*0.78283<<std::endl;
//       std::cout<<"ZDCP_HAD, going to fill some noise histos"<<std::endl;
                double ZDCP_HADNoise = HAD_Pnoise[iChannel-1]*0.78283;
                h_ZDCP_HAD_Noise[iChannel-1]->Fill(ZDCP_HADNoise);
                double ZDCP_HADEn = HAD_P_EnergyCheck[iChannel-1]*0.78283;
//                h_ZDCP_HAD_En[iChannel-1]->Fill(ZDCP_HADEn);
				}
				if (iSide == -1 ){
...
//				int chid = (iSection-1)*5+(iSide+1)/2*9+(iChannel-1); 
 //      std::cout<<"ZDCM_HAD, going to fill some noise histos"<<std::endl;
//				std::cout<<"RecHit energy = "<<zhit->energy()<<", Energy from Digi = "<<HAD_M_EnergyCheck[iChannel-1]*0.78283<<", Noise from Digi = "<<HAD_Mnoise[iChannel-1]*0.78283<<std::endl;
                double ZDCM_HADNoise = HAD_Mnoise[iChannel-1]*0.78283;
                h_ZDCM_HAD_Noise[iChannel-1]->Fill(ZDCM_HADNoise);
                double ZDCM_HADEn = HAD_M_EnergyCheck[iChannel-1]*0.78283;
//                h_ZDCM_HAD_En[iChannel-1]->Fill(ZDCM_HADEn);
				}
			}
		}
    	}
...
void MargueriteZDCAnalyzer::beginJob(){
	...
	h_ZDCP_EM_NoiseAve = book1DHistogram(ZDCDir,"h_ZDCP_EM_NoiseAve", "ZDCP EM Section Noise ZDCMrage per channel", 5, -0.5, 4.5);
	h_ZDCP_EM_NoiseAve->GetXaxis()->SetBinLabel(1,"+EM1");
	h_ZDCP_EM_NoiseAve->GetXaxis()->SetBinLabel(2,"+EM2");
	h_ZDCP_EM_NoiseAve->GetXaxis()->SetBinLabel(3,"+EM3");
	h_ZDCP_EM_NoiseAve->GetXaxis()->SetBinLabel(4,"+EM4");
	h_ZDCP_EM_NoiseAve->GetXaxis()->SetBinLabel(5,"+EM5");
	h_ZDCP_HAD_NoiseAve = book1DHistogram(ZDCDir,"h_ZDCP_HAD_NoiseAve", "ZDCP HAD Section Noise ZDCM per channel", 4, -0.5, 3.5);
	h_ZDCP_HAD_NoiseAve->GetXaxis()->SetBinLabel(1,"+HAD1");
	h_ZDCP_HAD_NoiseAve->GetXaxis()->SetBinLabel(2,"+HAD2");
	h_ZDCP_HAD_NoiseAve->GetXaxis()->SetBinLabel(3,"+HAD3");
	h_ZDCP_HAD_NoiseAve->GetXaxis()->SetBinLabel(4,"+HAD4");
	...
	h_ZDCP_EM_NoiseAve->Sumw2();
	h_ZDCP_HAD_NoiseAve->Sumw2();
	h_ZDCP_EM_NoiseAve->SetFillColor(kRed);
	h_ZDCP_HAD_NoiseAve->SetFillColor(kRed);
	...
	h_ZDCM_EM_NoiseAve = book1DHistogram(ZDCDir,"h_ZDCN_EM_NoiseAve", "ZDCM EM Section Noise average per channel", 5, -0.5, 4.5);
	h_ZDCM_EM_NoiseAve->GetXaxis()->SetBinLabel(1,"-EM1");
	h_ZDCM_EM_NoiseAve->GetXaxis()->SetBinLabel(2,"-EM2");
	h_ZDCM_EM_NoiseAve->GetXaxis()->SetBinLabel(3,"-EM3");
	h_ZDCM_EM_NoiseAve->GetXaxis()->SetBinLabel(4,"-EM4");
	h_ZDCM_EM_NoiseAve->GetXaxis()->SetBinLabel(5,"-EM5");
	h_ZDCM_HAD_NoiseAve = book1DHistogram(ZDCDir,"h_ZDCN_HAD_NoiseAve", "ZDCM HAD Section Noise average per channel", 4, -0.5, 3.5);
	h_ZDCM_HAD_NoiseAve->GetXaxis()->SetBinLabel(1,"-HAD1");
	h_ZDCM_HAD_NoiseAve->GetXaxis()->SetBinLabel(2,"-HAD2");
	h_ZDCM_HAD_NoiseAve->GetXaxis()->SetBinLabel(3,"-HAD3");
	h_ZDCM_HAD_NoiseAve->GetXaxis()->SetBinLabel(4,"-HAD4");
	...
	h_ZDCM_EM_NoiseAve->Sumw2();
	h_ZDCM_HAD_NoiseAve->Sumw2();
	h_ZDCM_EM_NoiseAve->SetFillColor(kRed);
	h_ZDCM_HAD_NoiseAve->SetFillColor(kRed);
...
bool MargueriteZDCAnalyzer::isGood(double fData[10], double fCut, double fPercentage){
	bool dec = false;
	int n = 10;
...
int MargueriteZDCAnalyzer::getTSMax(double fData[10], int fTS)
{
	int ts_max = -100;
	double max = -9999.;
	for(int j = 0; j < fTS; ++j){
...
double MargueriteZDCAnalyzer::getTime(double fData[10], double& fSum){
	int n = 10;
...
TH1F *MargueriteZDCAnalyzer::book1DHistogram(TFileDirectory & fDir, const std::string & fName, const std::string & fTitle,
...
TH2F *MargueriteZDCAnalyzer::book2DHistogram(TFileDirectory & fDir, const std::string & fName, const std::string & fTitle,
...
void MargueriteZDCAnalyzer::endJob() {
	for(int i = 0; i < 5;++i){
		...
		h_ZDCP_EM_NoiseAve->SetBinContent(i+1,h_ZDCP_EM_Noise[i]->GetMean());
		h_ZDCP_EM_NoiseAve->SetBinError(i+1,h_ZDCP_EM_Noise[i]->GetMeanError());
		h_ZDCM_EM_NoiseAve->SetBinContent(i+1,h_ZDCM_EM_Noise[i]->GetMean());
		h_ZDCM_EM_NoiseAve->SetBinError(i+1,h_ZDCM_EM_Noise[i]->GetMeanError());
	...
	for(int i = 0; i < 4;++i){
		...
		h_ZDCP_HAD_NoiseAve->SetBinContent(i+1,h_ZDCP_HAD_Noise[i]->GetMean());
		h_ZDCP_HAD_NoiseAve->SetBinError(i+1,h_ZDCP_HAD_Noise[i]->GetMeanError());
		h_ZDCM_HAD_NoiseAve->SetBinContent(i+1,h_ZDCM_HAD_Noise[i]->GetMean());
		h_ZDCM_HAD_NoiseAve->SetBinError(i+1,h_ZDCM_HAD_Noise[i]->GetMeanError());		
...
void MargueriteZDCAnalyzer::htmlOutput(void) 
{
	...
	if (runBegin != -1)
		sprintf(tmp, "ZDC_R%09ld_L%ld_%ld", runBegin,lumibegin,lumiend);
	else
		sprintf(tmp, "ZDC_R%09d", 0);
...
void MargueriteZDCAnalyzer::ZDCHTMLOutput(string startTime, string htmlDir, string htmlName) {
	...
	htmlFile << "<hr>" << endl;
	histoHTML(h_ZDCP_EM_TotE, "ZDCP EM Section Total Energy (GeV), +ZDC", "Events", 92, htmlFile, htmlDir,false);		
	histoHTML(h_ZDCP_Had_TotE, "ZDCP Had Section Total Energy (GeV), +ZDC", "Events", 92, htmlFile, htmlDir,false);		
	histoHTML(h_ZDCP_TotE_HadronLike, "ZDCP Total Energy Hadronic particle (GeV), +ZDC", "Events", 92, htmlFile, htmlDir,false);		
	histoHTML(h_ZDCP_TotE, "ZDCP Total EM+Had Rechit Energy(GeV), +ZDC", "Events", 92, htmlFile, htmlDir,false);		
	histoHTML(h_ZDCM_EM_TotE, "ZDCM EM Section Total Energy (GeV), -ZDC", "Events", 92, htmlFile, htmlDir,false);		
	histoHTML(h_ZDCM_Had_TotE, "ZDCM Had Section Total Energy (GeV), -ZDC", "Events", 92, htmlFile, htmlDir,false);		
	histoHTML(h_ZDCM_TotE_HadronLike, "ZDCM Total Energy Hadronic particle (GeV), -ZDC", "Events", 92, htmlFile, htmlDir,false);		
	histoHTML(h_ZDCM_TotE, "ZDCM Total EM+Had Rechit Energy(GeV), -ZDC", "Events", 92, htmlFile, htmlDir,false);		
	htmlFile << "<hr>" << endl;
	histoHTML(h_ZDCP_EM_NoiseAve, "ZDCP EM Section Average Noise (GeV), +ZDC", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_EM_NoiseAve, "ZDCM EM Section Average Noise (GeV), -ZDC", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_HAD_NoiseAve, "ZDCP HAD Section Average Noise (GeV), +ZDC", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_HAD_NoiseAve, "ZDCM HAD Section Average Noise (GeV), -ZDC", "Events", 92, htmlFile, htmlDir,false);

	histoHTML(h_ZDCP_EM_Noise[0], "ZDCP EM Channel1 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_EM_Noise[1], "ZDCP EM Channel2 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_EM_Noise[2], "ZDCP EM Channel3 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_EM_Noise[3], "ZDCP EM Channel4 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_EM_Noise[4], "ZDCP EM Channel5 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_HAD_Noise[0], "ZDCP HAD Channel1 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_HAD_Noise[1], "ZDCP HAD Channel2 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_HAD_Noise[2], "ZDCP HAD Channel3 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCP_HAD_Noise[3], "ZDCP HAD Channel4 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_EM_Noise[0], "ZDCM EM Channel1 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_EM_Noise[1], "ZDCM EM Channel2 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_EM_Noise[2], "ZDCM EM Channel3 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_EM_Noise[3], "ZDCM EM Channel4 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_EM_Noise[4], "ZDCM EM Channel5 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_HAD_Noise[0], "ZDCM HAD Channel1 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_HAD_Noise[1], "ZDCM HAD Channel2 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_HAD_Noise[2], "ZDCM HAD Channel3 Noise", "Events", 92, htmlFile, htmlDir,false);
	histoHTML(h_ZDCM_HAD_Noise[3], "ZDCM HAD Channel4 Noise", "Events", 92, htmlFile, htmlDir,false);	
...
void MargueriteZDCAnalyzer::histoHTML(TH1F * hist, const char *xlab, const char *ylab, int width, ofstream & htmlFile,
...
void MargueriteZDCAnalyzer::histoHTML2(TH2F * hist, const char *xlab, const char *ylab, int width,
...
string MargueriteZDCAnalyzer::getIMG(TH1F * hist, int size, string htmlDir, const char *xlab, const char *ylab, bool log) {
...
string MargueriteZDCAnalyzer::getIMG2(TH2F * hist, int size, string htmlDir, const char *xlab, const char *ylab,
  bool color, bool log) {
...
void MargueriteZDCAnalyzer::cleanString(string & title) {
...
void MargueriteZDCAnalyzer::parseString(string & title) {
