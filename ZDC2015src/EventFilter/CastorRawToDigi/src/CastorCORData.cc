...
void CastorCORData::dataPointers(const unsigned short** daq_first, 
...
void CastorCORData::determineSectionLengths(int& tpWords, int& daqWords, int& headerWords, int& trailerWords) const {
...
int CastorCORData::getFirmwareFlavor() const {
  return (m_formatVersion<2)?(-1):((m_rawConst[7]>>8)&0xFF);
}

uint32_t CastorCORData::zsBunchMask() const {
  uint32_t mask=0;
