// root -l -q -b RAA_plot_CutA_BasicPlots.C+
//MIT VERSION UPDATED May 19 2015 for MinBias subtraction MBT
#include <iostream>
#include <stdio.h>
#include <fstream>
#include <fstream>
#include <TH1F.h>
#include <TH1F.h>
#include <TH2F.h>
#include <TFile.h>
#include <TTree.h>
#include <TF1.h>
#include <TCanvas.h>
#include <TLegend.h>
#include <TGraphErrors.h>
#include <TGraphAsymmErrors.h>
#include <TH1.h>
#include <TH2.h>
#include <TH3.h>
#include <TFile.h>
#include <TStyle.h>
#include <TStopwatch.h>
#include <TRandom3.h>
#include <TChain.h>
#include <TProfile.h>
#include <TStopwatch.h>
#include <TCut.h>
#include <cstdlib>
#include <cmath>
#include "TLegend.h"
#include "TLatex.h"
#include "TMath.h"
#include "TLine.h"

#define pi 3.14159265

#include "Headers/plot.h"

static const int nbins_cent = 7;
static const Double_t boundaries_cent[nbins_cent+1] = {0,2,4,12,20,28,36,40};// multiply by 2.5 to get your actual centrality % (old 2011 data) 
//now we have to multiply by 5, since centrality goes from 0-200. 
int findBin(int bin)
{
  int ibin=-1;
  //! centrality is defined as 0.5% bins of cross section
  //! in 0-200 bins               
  if(bin<=10)ibin=0; //! 0-5%
  else if(bin>10  && bin<=20 )ibin=1; //! 5-10%
  else if(bin>20  && bin<=60 )ibin=2;  //! 10-30%
  else if(bin>60  && bin<=100)ibin=3;  //! 30-50%
  else if(bin>100 && bin<=140)ibin=4;  //! 50-70%
  else if(bin>140 && bin<=180)ibin=5;  //! 70-90%
  else if(bin>180 && bin<=200)ibin=6;  //! 90-100%
  return ibin;
}

using namespace std;

//static const int stdarr = 6;
static const int stdarray[6] = {1,2,3,4,5,6};

// Run with: 
// root -l RAA_plot_CutA_BasicPlots.C+
// gets a few warnings but runs ok
// Note you need Headers/plot.h in the directory
void RAA_plot_CutA_BasicPlots_tryMinB(Int_t radius = 3){

  TH1::SetDefaultSumw2();
  gStyle->SetOptStat(00000000);

  // the cut is a 3 step cut based on the different value of the calopt/pfpt - copy the following lines into your loop (with the corresponding branch address set)
  // if(calopt/pfpt <= 0.5 && eMax/Sumcand < 0.05) hGood->Fill();
  // if(calopt/pfpt > 0.5 && calopt/pfpt <= 0.85 && eMax/Sumcand < (18/7 *(Float_t)calopt_1/pfpt_1 - 9/7)) ) hGood->Fill();
  // if(calopt/pfpt > 0.85 & eMax/Sumcand > 0.9) hGood->Fill();
  
  Float_t ptCutMC = 15;
  Float_t ptCutData = 15;
  Float_t ptCutHydjetMC = 15;
  Float_t ptCutMinBiasData = 15;
  Float_t ptCutCentralData = 15;
  Float_t ptCutCentralMC = 15;
  
  
  char * etaWidth = (char*)"n20_eta_p20";
  TFile * fData, * fMC, *fMinBias, *fHydjet; 

  fData = TFile::Open("/mnt/hadoop/cms/store/user/pawan/ntuples/JetRaa_akPu234_PbPb_Data.root");
  fMC = TFile::Open("/mnt/hadoop/cms/store/user/pawan/ntuples/JetRaa_akPu234_PbPb_MC.root");
  fMinBias = TFile::Open("/export/d00/scratch/belt/condorfiles/pbpb/ntuples/JetRAA_DATA_PbPb_MinBias_ntuples.root");
 // TFile * fMinBias = TFile::Open(Form("Pawan_ntuple_PbPb_MinBiasData_spectra_JetID_CutA_finebins_CentralityWeightedAllMB_%s_R0p%d.root",etaWidth,radius)); //MinBias File 
 fHydjet = TFile::Open("/export/d00/scratch/pawan/condorfiles/pbpb/JetRaa_akPu234_PbPb_MinBias_MC.root");
 
  TTree * MinBias_matched= (TTree*)fMinBias->Get(Form("akPu%dJetAnalyzer/matchedJets",radius));
  TTree * MinBias_unmatched = (TTree*)fMinBias->Get(Form("akPu%dJetAnalyzer/unmatchedPFJets",radius));
    TTree * Hydjet_matched= (TTree*)fHydjet->Get(Form("akPu%dJetAnalyzer/matchedJets",radius));
  TTree * Hydjet_unmatched = (TTree*)fHydjet->Get(Form("akPu%dJetAnalyzer/unmatchedPFJets",radius));
  TFile *fcentin = TFile::Open("MinBiasHLTRatioWeightsVZ.root");
  TH1F *hCentWeight = (TH1F*)fcentin->Get("hBinEvents");
  
  cout<<"found the file"<<endl;

  TTree * Data_matched= (TTree*)fData->Get(Form("akPu%dJetAnalyzer/matchedJets",radius));
  TTree * Data_unmatched = (TTree*)fData->Get(Form("akPu%dJetAnalyzer/unmatchedPFJets",radius));

  TTree * MC_matched = (TTree*)fMC->Get(Form("akPu%dJetAnalyzer/matchedJets",radius));
  TTree * MC_unmatched = (TTree*)fMC->Get(Form("akPu%dJetAnalyzer/unmatchedPFJets",radius));


  TH1F * hMC_Jet55_noCut = new TH1F("hMC_Jet55_noCut","data from matched jets without any jet ID cut",400,0,400);
  TH1F * hMC_Jet55_CutA = new TH1F("hMC_Jet55_CutA","data from matched jets with Jet ID cut: slant line from 0.4 calopt/pfpt from eMax/Sumcand 0 till 0.9  and then calopt/pfpt > 0.85",400,0,400);
  TH1F * hMC_Jet55_CutA_rej = new TH1F("hMC_Jet55_CutA_rej","data from matched jets rejected by Jet ID cut: slant line from 0.4 calopt/pfpt from eMax/Sumcand 0 till 0.9  and then calopt/pfpt > 0.85",400,0,400);

  TH1F * hMC_Jet65_noCut = new TH1F("hMC_Jet65_noCut","data from matched jets without any jet ID cut",400,0,400);
  TH1F * hMC_Jet65_CutA = new TH1F("hMC_Jet65_CutA","data from matched jets with Jet ID cut: slant line from 0.4 calopt/pfpt from eMax/Sumcand 0 till 0.9  and then calopt/pfpt > 0.85",400,0,400);
  TH1F * hMC_Jet65_CutA_rej = new TH1F("hMC_Jet65_CutA_rej","data from matched jets rejected with Jet ID cut: slant line from 0.4 calopt/pfpt from eMax/Sumcand 0 till 0.9  and then calopt/pfpt > 0.85",400,0,400);

  TH1F * hMC_Jet80_noCut = new TH1F("hMC_Jet80_noCut","data from matched jets without any jet ID cut",400,0,400);
  TH1F * hMC_Jet80_CutA = new TH1F("hMC_Jet80_CutA","data from matched jets with Jet ID cut: slant line from 0.4 calopt/pfpt from eMax/Sumcand 0 till 0.9  and then calopt/pfpt > 0.85",400,0,400);
  TH1F * hMC_Jet80_CutA_rej = new TH1F("hMC_Jet80_CutA_rej","data from matched jets rejected with Jet ID cut: slant line from 0.4 calopt/pfpt from eMax/Sumcand 0 till 0.9  and then calopt/pfpt > 0.85",400,0,400);

  TH1F * hData_Jet55_noCut = new TH1F("hData_Jet55_noCut","data from matched jets without any jet ID cut",400,0,400);
  TH1F * hData_Jet55_CutA = new TH1F("hData_Jet55_CutA","data from matched jets with Jet ID cut: slant line from 0.5 calopt/pfpt from eMax/Sumcand 0 till 1 and then calopt/pfpt > 0.85",400,0,400);
  // this cut is a combination of two: 
  // if(eMax/SumCand < 0.9 && ( eMax/Sumcand < (18/7 calopt/pfpt - 9/7) ) ) { hData_Jet55_CutA->Fill()}
  // if { eMax/Sumcand >0.9 && calopt/pfpt > 0.85} { hData_Jet55_CutA->Fill()}  
  // this cut is if( eMax/pfpt < (22/15 * calopt/pfpt - 11/15) ) {hData_Jet55_CutB->Fill()} 
  TH1F * hData_Jet55_CutA_rej = new TH1F("hData_Jet55_CutA_rej","",400,0,400);

  TH1F * hData_Jet65_noCut = new TH1F("hData_Jet65_noCut","data from matched jets without any jet ID cut",400,0,400);
  TH1F * hData_Jet65_CutA = new TH1F("hData_Jet65_CutA","data from matched jets with Jet ID cut: slant line from 0.5 calopt/pfpt from eMax/Sumcand 0 till 1 and then calopt/pfpt > 0.85",400,0,400);
  // this cut is a combination of two: 
  // if(eMax/SumCand < 0.9 && ( eMax/Sumcand < (18/7 calopt/pfpt - 9/7) ) ) { hData_Jet65_CutA->Fill()}
  // if { eMax/Sumcand >0.9 && calopt/pfpt > 0.85} { hData_Jet65_CutA->Fill()}  
  // this cut is if( eMax/pfpt < (22/15 * calopt/pfpt - 11/15) ) {hData_Jet65_CutB->Fill()} 
  TH1F * hData_Jet65_CutA_rej = new TH1F("hData_Jet65_CutA_rej","",400,0,400);
  
  TH1F * hData_Jet80_noCut = new TH1F("hData_Jet80_noCut","data from matched jets without any jet ID cut",400,0,400);
  TH1F * hData_Jet80_CutA = new TH1F("hData_Jet80_CutA","data from matched jets with Jet ID cut: slant line from 0.5 calopt/pfpt from eMax/Sumcand 0 till 1 and then calopt/pfpt > 0.85",400,0,400);
  // this cut is a combination of two: 
  // if(eMax/SumCand < 0.9 && ( eMax/Sumcand < (18/7 calopt/pfpt - 9/7) ) ) { hData_Jet80_CutA->Fill()}
  // if { eMax/Sumcand >0.9 && calopt/pfpt > 0.85} { hData_Jet80_CutA->Fill()}  
  // this cut is if( eMax/pfpt < (22/15 * calopt/pfpt - 11/15) ) {hData_Jet80_CutB->Fill()} 
  TH1F * hData_Jet80_CutA_rej = new TH1F("hData_Jet80_CutA_rej","",400,0,400);

  TH1F * hData_unmatched_Jet80_noCut = new TH1F("hData_unmatched_Jet80_noCut","",400,0,400);
  TH1F * hData_unmatched_Jet80_CutA = new TH1F("hData_unmatched_Jet80_CutA","",400,0,400);
  TH1F * hData_unmatched_Jet80_CutA_rej = new TH1F("hData_unmatched_Jet80_CutA_rej","",400,0,400);
  TH1F * hData_unmatched_Jet65_noCut = new TH1F("hData_unmatched_Jet65_noCut","",400,0,400);
  TH1F * hData_unmatched_Jet65_CutA = new TH1F("hData_unmatched_Jet65_CutA","",400,0,400);
  TH1F * hData_unmatched_Jet65_CutA_rej = new TH1F("hData_unmatched_Jet65_CutA_rej","",400,0,400);
  TH1F * hData_unmatched_Jet55_noCut = new TH1F("hData_unmatched_Jet55_noCut","",400,0,400);
  TH1F * hData_unmatched_Jet55_CutA = new TH1F("hData_unmatched_Jet55_CutA","",400,0,400);
  TH1F * hData_unmatched_Jet55_CutA_rej = new TH1F("hData_unmatched_Jet55_CutA_rej","",400,0,400);

  TH1F * hMC_unmatched_Jet80_noCut = new TH1F("hMC_unmatched_Jet80_noCut","",400,0,400);
  TH1F * hMC_unmatched_Jet80_CutA = new TH1F("hMC_unmatched_Jet80_CutA","",400,0,400);
  TH1F * hMC_unmatched_Jet80_CutA_rej = new TH1F("hMC_unmatched_Jet80_CutA_rej","",400,0,400);
  TH1F * hMC_unmatched_Jet65_noCut = new TH1F("hMC_unmatched_Jet65_noCut","",400,0,400);
  TH1F * hMC_unmatched_Jet65_CutA = new TH1F("hMC_unmatched_Jet65_CutA","",400,0,400);
  TH1F * hMC_unmatched_Jet65_CutA_rej = new TH1F("hMC_unmatched_Jet65_CutA_rej","",400,0,400);
  TH1F * hMC_unmatched_Jet55_noCut = new TH1F("hMC_unmatched_Jet55_noCut","",400,0,400);
  TH1F * hMC_unmatched_Jet55_CutA = new TH1F("hMC_unmatched_Jet55_CutA","",400,0,400);
  TH1F * hMC_unmatched_Jet55_CutA_rej = new TH1F("hMC_unmatched_Jet55_CutA_rej","",400,0,400);

 
  //get the spectra with the specific trigger object from the different files. 
  TH1F *hpbpb_Jet80_gen[nbins_cent],*hpbpb_Jet80_reco[nbins_cent];
  TH1F *hpbpb_Jet65_gen[nbins_cent],*hpbpb_Jet65_reco[nbins_cent];
  TH1F *hpbpb_Jet55_gen[nbins_cent],*hpbpb_Jet55_reco[nbins_cent];
  TH1F *hpbpb_JetComb_gen[nbins_cent],*hpbpb_JetComb_reco[nbins_cent];

  //TH1F *hpbpb_gen[nbins_cent],*hpbpb_reco[nbins_cent];
  //TH2F *hpbpb_matrix[nbins_cent];
  TH2F *hpbpb_matrix_HLT[nbins_cent];
  //TH2F *hpbpb_mcclosure_matrix[nbins_cent];
  TH2F *hpbpb_mcclosure_matrix_HLT[nbins_cent];
  //TH2F *hpbpb_response[nbins_cent];
  TH1F *hpbpb_mcclosure_JetComb_data[nbins_cent];
  //TH1F *hpbpb_mcclosure_data[nbins_cent];
  TH1F *hpbpb_mcclosure_Jet80_data[nbins_cent];
  TH1F *hpbpb_mcclosure_Jet65_data[nbins_cent];
  TH1F *hpbpb_mcclosure_Jet55_data[nbins_cent];
  //TH1F *hpbpb_mcclosure_gen[nbins_cent];
  TH1F *hpbpb_mcclosure_JetComb_gen[nbins_cent];
  TH1F *hpbpb_mcclosure_Jet80_gen[nbins_cent];
  TH1F *hpbpb_mcclosure_Jet65_gen[nbins_cent];
  TH1F *hpbpb_mcclosure_Jet55_gen[nbins_cent];

  TH1F *hpbpb_TrgObj80[nbins_cent];
  TH1F *hpbpb_TrgObj65[nbins_cent];
  TH1F *hpbpb_TrgObj55[nbins_cent];
  TH1F *hpbpb_TrgObjComb[nbins_cent];
 //begin new plots
 // centrality based
    TH1F * hData_noCut_eta_cent[nbins_cent];
   TH1F * hData_CutA_eta_cent[nbins_cent];
  TH1F * hData_Jet55_noCut_eta_cent[nbins_cent];
  TH1F * hData_Jet55_CutA_eta_cent[nbins_cent];
  TH1F * hData_Jet65_noCut_eta_cent[nbins_cent];
  TH1F * hData_Jet65_CutA_eta_cent[nbins_cent];
  TH1F * hData_Jet80_noCut_eta_cent[nbins_cent];
  TH1F * hData_Jet80_CutA_eta_cent[nbins_cent];

  TH1F * hData_noCut_phi_cent[nbins_cent];
  TH1F * hData_CutA_phi_cent[nbins_cent];
  TH1F * hData_Jet55_noCut_phi_cent[nbins_cent];
  TH1F * hData_Jet55_CutA_phi_cent[nbins_cent];
  TH1F * hData_Jet65_noCut_phi_cent[nbins_cent];
  TH1F * hData_Jet65_CutA_phi_cent[nbins_cent];
  TH1F * hData_Jet80_noCut_phi_cent[nbins_cent];
  TH1F * hData_Jet80_CutA_phi_cent[nbins_cent];

   TH1F * hMC_noCut_eta_cent[nbins_cent];
   TH1F * hMC_CutA_eta_cent[nbins_cent];
  TH1F * hMC_Jet55_noCut_eta_cent[nbins_cent];
  TH1F * hMC_Jet55_CutA_eta_cent[nbins_cent];
  TH1F * hMC_Jet65_noCut_eta_cent[nbins_cent];
  TH1F * hMC_Jet65_CutA_eta_cent[nbins_cent];
  TH1F * hMC_Jet80_noCut_eta_cent[nbins_cent];
  TH1F * hMC_Jet80_CutA_eta_cent[nbins_cent];

  TH1F * hMC_noCut_phi_cent[nbins_cent];
  TH1F * hMC_CutA_phi_cent[nbins_cent];
  TH1F * hMC_Jet55_noCut_phi_cent[nbins_cent];
  TH1F * hMC_Jet55_CutA_phi_cent[nbins_cent];
  TH1F * hMC_Jet65_noCut_phi_cent[nbins_cent];
  TH1F * hMC_Jet65_CutA_phi_cent[nbins_cent];
  TH1F * hMC_Jet80_noCut_phi_cent[nbins_cent];
  TH1F * hMC_Jet80_CutA_phi_cent[nbins_cent];
  
 // centrality based
    TH1F * hMinBiasData_noCut_eta_cent[nbins_cent];
   TH1F * hMinBiasData_CutA_eta_cent[nbins_cent];
  TH1F * hMinBiasData_Jet55_noCut_eta_cent[nbins_cent];
  TH1F * hMinBiasData_Jet55_CutA_eta_cent[nbins_cent];
  TH1F * hMinBiasData_Jet65_noCut_eta_cent[nbins_cent];
  TH1F * hMinBiasData_Jet65_CutA_eta_cent[nbins_cent];
  TH1F * hMinBiasData_Jet80_noCut_eta_cent[nbins_cent];
  TH1F * hMinBiasData_Jet80_CutA_eta_cent[nbins_cent];

  TH1F * hMinBiasData_noCut_phi_cent[nbins_cent];
  TH1F * hMinBiasData_CutA_phi_cent[nbins_cent];
  TH1F * hMinBiasData_Jet55_noCut_phi_cent[nbins_cent];
  TH1F * hMinBiasData_Jet55_CutA_phi_cent[nbins_cent];
  TH1F * hMinBiasData_Jet65_noCut_phi_cent[nbins_cent];
  TH1F * hMinBiasData_Jet65_CutA_phi_cent[nbins_cent];
  TH1F * hMinBiasData_Jet80_noCut_phi_cent[nbins_cent];
  TH1F * hMinBiasData_Jet80_CutA_phi_cent[nbins_cent];

   TH1F * hHydjetMC_noCut_eta_cent[nbins_cent];
   TH1F * hHydjetMC_CutA_eta_cent[nbins_cent];
  TH1F * hHydjetMC_Jet55_noCut_eta_cent[nbins_cent];
  TH1F * hHydjetMC_Jet55_CutA_eta_cent[nbins_cent];
  TH1F * hHydjetMC_Jet65_noCut_eta_cent[nbins_cent];
  TH1F * hHydjetMC_Jet65_CutA_eta_cent[nbins_cent];
  TH1F * hHydjetMC_Jet80_noCut_eta_cent[nbins_cent];
  TH1F * hHydjetMC_Jet80_CutA_eta_cent[nbins_cent];

  TH1F * hHydjetMC_noCut_phi_cent[nbins_cent];
  TH1F * hHydjetMC_CutA_phi_cent[nbins_cent];
  TH1F * hHydjetMC_Jet55_noCut_phi_cent[nbins_cent];
  TH1F * hHydjetMC_Jet55_CutA_phi_cent[nbins_cent];
  TH1F * hHydjetMC_Jet65_noCut_phi_cent[nbins_cent];
  TH1F * hHydjetMC_Jet65_CutA_phi_cent[nbins_cent];
  TH1F * hHydjetMC_Jet80_noCut_phi_cent[nbins_cent];
  TH1F * hHydjetMC_Jet80_CutA_phi_cent[nbins_cent];
  
  TH2F * hMinBiasData_Jet55_noCut_JetpTeta[nbins_cent];
  TH2F * hMinBiasData_Jet55_CutA_JetpTeta[nbins_cent];
  TH2F * hMinBiasData_Jet65_noCut_JetpTeta[nbins_cent];
  TH2F * hMinBiasData_Jet65_CutA_JetpTeta[nbins_cent];
  TH2F * hMinBiasData_Jet80_noCut_JetpTeta[nbins_cent];
  TH2F * hMinBiasData_Jet80_CutA_JetpTeta[nbins_cent];
  TH2F * hHydjetMC_Jet55_noCut_JetpTeta[nbins_cent];
  TH2F * hHydjetMC_Jet55_CutA_JetpTeta[nbins_cent];
  TH2F * hHydjetMC_Jet65_noCut_JetpTeta[nbins_cent];
  TH2F * hHydjetMC_Jet65_CutA_JetpTeta[nbins_cent];
  TH2F * hHydjetMC_Jet80_noCut_JetpTeta[nbins_cent];
  TH2F * hHydjetMC_Jet80_CutA_JetpTeta[nbins_cent];
    
// non-centrality based
   TH1F * hData_noCut_eta = new TH1F("hData_noCut_eta","",100,-2.5,2.5);
   TH1F * hData_CutA_eta = new TH1F("hData_CutA_eta","",100,-2.5,2.5);

  TH1F * hData_Jet55_noCut_eta = new TH1F("hData_Jet55_noCut_eta","",50,-2.5,2.5);
  TH1F * hData_Jet55_CutA_eta = new TH1F("hData_Jet55_CutA_eta","",50,-2.5,2.5);
  TH1F * hData_Jet65_noCut_eta = new TH1F("hData_Jet65_noCut_eta","",50,-2.5,2.5);
  TH1F * hData_Jet65_CutA_eta = new TH1F("hData_Jet65_CutA_eta","",50,-2.5,2.5);
  TH1F * hData_Jet80_noCut_eta = new TH1F("hData_Jet80_noCut_eta","",50,-2.5,2.5);
  TH1F * hData_Jet80_CutA_eta = new TH1F("hData_Jet80_CutA_eta","",50,-2.5,2.5);

  TH1F * hData_noCut_phi = new TH1F("hData_noCut_phi","",50,-pi,pi);
  TH1F * hData_CutA_phi = new TH1F("hData_CutA_phi","",50,-pi,pi);

  TH1F * hData_Jet55_noCut_phi = new TH1F("hData_Jet55_noCut_phi","",50,-pi,pi);
  TH1F * hData_Jet55_CutA_phi = new TH1F("hData_Jet55_CutA_phi","",50,-pi,pi);
  TH1F * hData_Jet65_noCut_phi = new TH1F("hData_Jet65_noCut_phi","",50,-pi,pi);
  TH1F * hData_Jet65_CutA_phi = new TH1F("hData_Jet65_CutA_phi","",50,-pi,pi);
  TH1F * hData_Jet80_noCut_phi = new TH1F("hData_Jet80_noCut_phi","",50,-pi,pi);
  TH1F * hData_Jet80_CutA_phi = new TH1F("hData_Jet80_CutA_phi","",50,-pi,pi);
  
  TH2F * hData_noCut_phi_eta = new TH2F("hData_noCut_phi_eta","",100,-2.5,2.5,100,-pi,pi);
  TH2F * hData_CutA_phi_eta = new TH2F("hData_CutA_phi_eta","",100,-2.5,2.5,100,-pi,pi);

   TH1F * hMC_noCut_eta = new TH1F("hMC_noCut_eta","",50,-2.5,2.5);
   TH1F * hMC_CutA_eta = new TH1F("hMC_CutA_eta","",50,-2.5,2.5);

  TH1F * hMC_Jet55_noCut_eta = new TH1F("hMC_Jet55_noCut_eta","",50,-2.5,2.5);
  TH1F * hMC_Jet55_CutA_eta = new TH1F("hMC_Jet55_CutA_eta","",50,-2.5,2.5);
  TH1F * hMC_Jet65_noCut_eta = new TH1F("hMC_Jet65_noCut_eta","",50,-2.5,2.5);
  TH1F * hMC_Jet65_CutA_eta = new TH1F("hMC_Jet65_CutA_eta","",50,-2.5,2.5);
  TH1F * hMC_Jet80_noCut_eta = new TH1F("hMC_Jet80_noCut_eta","",50,-2.5,2.5);
  TH1F * hMC_Jet80_CutA_eta = new TH1F("hMC_Jet80_CutA_eta","",50,-2.5,2.5);

  TH1F * hMC_noCut_phi = new TH1F("hMC_noCut_phi","",50,-pi,pi);
  TH1F * hMC_CutA_phi = new TH1F("hMC_CutA_phi","",50,-pi,pi);

  TH1F * hMC_Jet55_noCut_phi = new TH1F("hMC_Jet55_noCut_phi","",50,-pi,pi);
  TH1F * hMC_Jet55_CutA_phi = new TH1F("hMC_Jet55_CutA_phi","",50,-pi,pi);
  TH1F * hMC_Jet65_noCut_phi = new TH1F("hMC_Jet65_noCut_phi","",50,-pi,pi);
  TH1F * hMC_Jet65_CutA_phi = new TH1F("hMC_Jet65_CutA_phi","",50,-pi,pi);
  TH1F * hMC_Jet80_noCut_phi = new TH1F("hMC_Jet80_noCut_phi","",50,-pi,pi);
  TH1F * hMC_Jet80_CutA_phi = new TH1F("hMC_Jet80_CutA_phi","",50,-pi,pi);
  
  TH2F * hMC_noCut_phi_eta = new TH2F("hMC_noCut_phi_eta","",100,-2.5,2.5,100,-pi,pi);
  TH2F * hMC_CutA_phi_eta = new TH2F("hMC_CutA_phi_eta","",100,-2.5,2.5,100,-pi,pi);
    
  TCanvas  * cmatrix_HTL[nbins_cent]; // uses refpt- MC only
  TCanvas  * cPbPb_pteta_noCut[nbins_cent];
  TCanvas  * cPbPb_pteta_CutA[nbins_cent];
  TCanvas  * cMC_pteta_noCut[nbins_cent];
  TCanvas  * cMC_pteta_CutA[nbins_cent];

  TCanvas * cPbPb_pteta80_noCut = new TCanvas(Form("hData_noCut_pTeta80_R%d_allcent",radius),Form("hData_noCut_pTeta80_R%d_allcent",radius),1500,1400);
  TCanvas * cPbPb_pteta80_CutA = new TCanvas(Form("hData_noCut_pTeta80_R%d_allcent",radius),Form("hData_noCut_pTeta80_R%d_allcent",radius),1500,1400);  
  TCanvas * cMC_pteta80_noCut = new TCanvas(Form("hMC_noCut_pTeta80_R%d_allcent",radius),Form("hMC_noCut_pTeta80_R%d_allcent",radius),1500,1400);
  TCanvas * cMC_pteta80_CutA = new TCanvas(Form("hMC_CutA_pTeta80_R%d_allcent",radius),Form("hMC_CutA_pTeta80_R%d_allcent",radius),1500,1400);
 
  TCanvas * cphieta_noCut[nbins_cent];
  TCanvas * cphieta_CutA[nbins_cent];
  
  TCanvas * cetacan_CutA[nbins_cent];
  TCanvas * cetacan_noCut[nbins_cent];
  TCanvas * cphican_CutA[nbins_cent];
  TCanvas * cphican_noCut[nbins_cent];
//new plots

TH2F * hData_Jet55_noCut_JetpTeta[nbins_cent];
TH2F * hData_Jet55_CutA_JetpTeta[nbins_cent];
TH2F * hData_Jet65_noCut_JetpTeta[nbins_cent];
TH2F * hData_Jet65_CutA_JetpTeta[nbins_cent];
TH2F * hData_Jet80_noCut_JetpTeta[nbins_cent];
TH2F * hData_Jet80_CutA_JetpTeta[nbins_cent];

TH2F * hMC_Jet55_noCut_JetpTeta[nbins_cent];
TH2F * hMC_Jet55_CutA_JetpTeta[nbins_cent];
TH2F * hMC_Jet65_noCut_JetpTeta[nbins_cent];
TH2F * hMC_Jet65_CutA_JetpTeta[nbins_cent];
TH2F * hMC_Jet80_noCut_JetpTeta[nbins_cent];
TH2F * hMC_Jet80_CutA_JetpTeta[nbins_cent];

cout<<"histograms declared"<<endl;	
	
  for(int i = 0;i<nbins_cent;++i){
    //cout<<"cent bin = "<<i<<endl;

    hpbpb_TrgObj80[i] = new TH1F(Form("hpbpb_HLT80_R%d_%s_cent%d",radius,etaWidth,i),Form("Spectra from  Jet 80 R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_TrgObj65[i] = new TH1F(Form("hpbpb_HLT65_R%d_%s_cent%d",radius,etaWidth,i),Form("Spectra from  Jet 65 && !jet80 R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_TrgObj55[i] = new TH1F(Form("hpbpb_HLT55_R%d_%s_cent%d",radius,etaWidth,i),Form("Spectra from Jet 55 && !jet65 && !jet80 R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_TrgObjComb[i] = new TH1F(Form("hpbpb_HLTComb_R%d_%s_cent%d",radius,etaWidth,i),Form("Trig Combined Spectra R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
	hpbpb_matrix_HLT[i] = new TH2F(Form("hpbpb_matrix_HLT_R%d_%s_cent%d",radius,etaWidth,i),Form("Matrix refpt jtpt from trigger addition R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400,400,0,400); //<- MATRIX HLT
    //hpbpb_gen[i] = new TH1F(Form("hpbpb_gen_R%d_%s_cent%d",radius,etaWidth,i),Form("Gen refpt R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    //cout<<"A"<<endl;
    //hpbpb_reco[i] = new TH1F(Form("hpbpb_reco_R%d_%s_cent%d",radius,etaWidth,i),Form("Reco jtpt R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    //cout<<"B"<<endl;
    //hpbpb_matrix[i] = new TH2F(Form("hpbpb_matrix_R%d_%s_cent%d",radius,etaWidth,i),Form("Matrix refpt jtpt R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400,400,0,400);
    
    //hpbpb_mcclosure_matrix[i] = new TH2F(Form("hpbpb_mcclosure_matrix_R%d_%s_cent%d",radius,etaWidth,i),Form("Matrix for mcclosure refpt jtpt R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400,400,0,400);
    hpbpb_mcclosure_matrix_HLT[i] = new TH2F(Form("hpbpb_mcclosure_matrix_HLT_R%d_%s_cent%d",radius,etaWidth,i),Form("Matrix for mcclosure refpt jtpt from Jet triggers R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400,400,0,400);
    //cout<<"C"<<endl;
    //hpbpb_mcclosure_data[i] = new TH1F(Form("hpbpb_mcclosure_data_R%d_%s_cent%d",radius,etaWidth,i),Form("data for unfolding mc closure test R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_JetComb_data[i] = new TH1F(Form("hpbpb_mcclosure_JetComb_data_R%d_%s_cent%d",radius,etaWidth,i),Form("data for unfolding mc closure test trigger combined  R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_Jet80_data[i] = new TH1F(Form("hpbpb_mcclosure_Jet80_data_R%d_%s_cent%d",radius,etaWidth,i),Form("data for unfolding mc closure test trigger 80  R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_Jet65_data[i] = new TH1F(Form("hpbpb_mcclosure_Jet65_data_R%d_%s_cent%d",radius,etaWidth,i),Form("data for unfolding mc closure test trigger 65  R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_Jet55_data[i] = new TH1F(Form("hpbpb_mcclosure_Jet55_data_R%d_%s_cent%d",radius,etaWidth,i),Form("data for unfolding mc closure test trigger 55  R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);

    //hpbpb_mcclosure_gen[i] = new TH1F(Form("hpbpb_mcclosure_gen_R%d_%s_cent%d",radius,etaWidth,i),Form("gen spectra for unfolding mc closure test R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_JetComb_gen[i] = new TH1F(Form("hpbpb_mcclosure_gen_JetComb_R%d_%s_cent%d",radius,etaWidth,i),Form("gen spectra for unfolding mc closure test trigger combined R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_Jet80_gen[i] = new TH1F(Form("hpbpb_mcclosure_gen_Jet80_R%d_%s_cent%d",radius,etaWidth,i),Form("gen spectra for unfolding mc closure test trigger 80 R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_Jet65_gen[i] = new TH1F(Form("hpbpb_mcclosure_gen_Jet65_R%d_%s_cent%d",radius,etaWidth,i),Form("gen spectra for unfolding mc closure test trigger 65 R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_mcclosure_Jet55_gen[i] = new TH1F(Form("hpbpb_mcclosure_gen_Jet55_R%d_%s_cent%d",radius,etaWidth,i),Form("gen spectra for unfolding mc closure test trigger 55 R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);

    hpbpb_JetComb_gen[i] = new TH1F(Form("hpbpb_JetComb_gen_R%d_%s_cent%d",radius,etaWidth,i),Form("Gen refpt from HLT trigger combined R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_JetComb_reco[i] = new TH1F(Form("hpbpb_JetComb_reco_R%d_%s_cent%d",radius,etaWidth,i),Form("reco jtpt from HLT trigger combined R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_Jet80_gen[i] = new TH1F(Form("hpbpb_Jet80_gen_R%d_%s_cent%d",radius,etaWidth,i),Form("Gen refpt from Jet80 trigger R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_Jet80_reco[i] = new TH1F(Form("hpbpb_Jet80_reco_R%d_%s_cent%d",radius,etaWidth,i),Form("reco jtpt from Jet80 trigger R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_Jet65_gen[i] = new TH1F(Form("hpbpb_Jet65_gen_R%d_%s_cent%d",radius,etaWidth,i),Form("Gen refpt from Jet65 && !Jet80 trigger R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_Jet65_reco[i] = new TH1F(Form("hpbpb_Jet65_reco_R%d_%s_cent%d",radius,etaWidth,i),Form("reco jtpt from Jet65 && !Jet80 trigger R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_Jet55_gen[i] = new TH1F(Form("hpbpb_Jet55_gen_R%d_%s_cent%d",radius,etaWidth,i),Form("Gen refpt from Jet55 && !Jet65 && !Jet80 trigger R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
    hpbpb_Jet55_reco[i] = new TH1F(Form("hpbpb_Jet55_reco_R%d_%s_cent%d",radius,etaWidth,i),Form("reco jtpt from Jet55 && !Jet65 && !Jet80 trigger R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400);
// new plots

    hData_noCut_eta_cent[i] = new TH1F(Form("hData_noCut_eta_R%d_cent%d",radius,i),Form("Data no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
    hData_CutA_eta_cent[i] = new TH1F(Form("hData_CutA_eta_R%d_cent%d",radius,i),Form("Data CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet55_noCut_eta_cent[i] = new TH1F(Form("hData_Jet55_noCut_eta_R%d_cent%d",radius,i),Form("Data Jet55 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet55_CutA_eta_cent[i] = new TH1F(Form("hData_Jet55_CutA_eta_R%d_cent%d",radius,i),Form("Data Jet55 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet65_noCut_eta_cent[i] = new TH1F(Form("hData_Jet65_noCut_eta_R%d_cent%d",radius,i),Form("Data Jet65 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet65_CutA_eta_cent[i] = new TH1F(Form("hData_Jet65_CutA_eta_R%d_cent%d",radius,i),Form("Data Jet65 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet80_noCut_eta_cent[i] = new TH1F(Form("hData_Jet80_noCut_eta_R%d_cent%d",radius,i),Form("Data Jet80 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet80_CutA_eta_cent[i] = new TH1F(Form("hData_Jet80_CutA_eta_R%d_cent%d",radius,i),Form("Data Jet80 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);

    hData_noCut_phi_cent[i] = new TH1F(Form("hData_noCut_phi_R%d_cent%d",radius,i),Form("Data no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
    hData_CutA_phi_cent[i] = new TH1F(Form("hData_CutA_phi_R%d_cent%d",radius,i),Form("Data CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet55_noCut_phi_cent[i] = new TH1F(Form("hData_Jet55_noCut_phi_R%d_cent%d",radius,i),Form("Data Jet55 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet55_CutA_phi_cent[i] = new TH1F(Form("hData_Jet55_CutA_phi_R%d_cent%d",radius,i),Form("Data Jet55 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet65_noCut_phi_cent[i] = new TH1F(Form("hData_Jet65_noCut_phi_R%d_cent%d",radius,i),Form("Data Jet65 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet65_CutA_phi_cent[i] = new TH1F(Form("hData_Jet65_CutA_phi_R%d_cent%d",radius,i),Form("Data Jet65 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet80_noCut_phi_cent[i] = new TH1F(Form("hData_Jet80_noCut_phi_R%d_cent%d",radius,i),Form("Data Jet80 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hData_Jet80_CutA_phi_cent[i] = new TH1F(Form("hData_Jet80_CutA_phi_R%d_cent%d",radius,i),Form("Data Jet80 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);

    hMC_noCut_eta_cent[i] = new TH1F(Form("hMC_noCut_eta_R%d_cent%d",radius,i),Form("MC no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
    hMC_CutA_eta_cent[i] = new TH1F(Form("hMC_CutA_eta_R%d_cent%d",radius,i),Form("MC CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMC_Jet55_noCut_eta_cent[i] = new TH1F(Form("hMC_Jet55_noCut_eta_R%d_cent%d",radius,i),Form("MC Jet55 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMC_Jet55_CutA_eta_cent[i] = new TH1F(Form("hMC_Jet55_CutA_eta_R%d_cent%d",radius,i),Form("MC Jet55 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMC_Jet65_noCut_eta_cent[i] = new TH1F(Form("hMC_Jet65_noCut_eta_R%d_cent%d",radius,i),Form("MC Jet65 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMC_Jet65_CutA_eta_cent[i] = new TH1F(Form("hMC_Jet65_CutA_eta_R%d_cent%d",radius,i),Form("MC Jet65 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMC_Jet80_noCut_eta_cent[i] = new TH1F(Form("hMC_Jet80_noCut_eta_R%d_cent%d",radius,i),Form("MC Jet80 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMC_Jet80_CutA_eta_cent[i] = new TH1F(Form("hMC_Jet80_CutA_eta_R%d_cent%d",radius,i),Form("MC Jet80 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);

    hMC_noCut_phi_cent[i] = new TH1F(Form("hMC_noCut_phi_R%d_cent%d",radius,i),Form("MC no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
    hMC_CutA_phi_cent[i] = new TH1F(Form("hMC_CutA_phi_R%d_cent%d",radius,i),Form("MC CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hMC_Jet55_noCut_phi_cent[i] = new TH1F(Form("hMC_Jet55_noCut_phi_R%d_cent%d",radius,i),Form("MC Jet55 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hMC_Jet55_CutA_phi_cent[i] = new TH1F(Form("hMC_Jet55_CutA_phi_R%d_cent%d",radius,i),Form("MC Jet55 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hMC_Jet65_noCut_phi_cent[i] = new TH1F(Form("hMC_Jet65_noCut_phi_R%d_cent%d",radius,i),Form("MC Jet65 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hMC_Jet65_CutA_phi_cent[i] = new TH1F(Form("hMC_Jet65_CutA_phi_R%d_cent%d",radius,i),Form("MC Jet65 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hMC_Jet80_noCut_phi_cent[i] = new TH1F(Form("hMC_Jet80_noCut_phi_R%d_cent%d",radius,i),Form("MC Jet80 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hMC_Jet80_CutA_phi_cent[i] = new TH1F(Form("hMC_Jet80_CutA_phi_R%d_cent%d",radius,i),Form("MC Jet80 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);

   hData_Jet55_noCut_JetpTeta[i] = new TH2F(Form("hData_Jet55_noCut_pTeta_R%d_cent%d",radius,i),Form("Data Jet55 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hData_Jet55_CutA_JetpTeta[i] = new TH2F(Form("hData_Jet55_CutA_pTeta_R%d_cent%d",radius,i),Form("Data Jet55 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hData_Jet65_noCut_JetpTeta[i] = new TH2F(Form("hData_Jet65_noCut_pTeta_R%d_cent%d",radius,i),Form("Data Jet65 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hData_Jet65_CutA_JetpTeta[i] = new TH2F(Form("hData_Jet65_CutA_pTeta_R%d_cent%d",radius,i),Form("Data Jet65 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hData_Jet80_noCut_JetpTeta[i] = new TH2F(Form("hData_Jet80_noCut_pTeta_R%d_cent%d",radius,i),Form("Data Jet80 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hData_Jet80_CutA_JetpTeta[i] = new TH2F(Form("hData_Jet80_CutA_pTeta_R%d_cent%d",radius,i),Form("Data Jet80 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);

   hMC_Jet55_noCut_JetpTeta[i] = new TH2F(Form("hMC_Jet55_noCut_pTeta_R%d_cent%d",radius,i),Form("MC Jet55 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hMC_Jet55_CutA_JetpTeta[i] = new TH2F(Form("hMC_Jet55_CutA_pTeta_R%d_cent%d",radius,i),Form("MC Jet55 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hMC_Jet65_noCut_JetpTeta[i] = new TH2F(Form("hMC_Jet65_noCut_pTeta_R%d_cent%d",radius,i),Form("MC Jet65 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hMC_Jet65_CutA_JetpTeta[i] = new TH2F(Form("hMC_Jet65_CutA_pTeta_R%d_cent%d",radius,i),Form("MC Jet65 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hMC_Jet80_noCut_JetpTeta[i] = new TH2F(Form("hMC_Jet80_noCut_pTeta_R%d_cent%d",radius,i),Form("MC Jet80 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hMC_Jet80_CutA_JetpTeta[i] = new TH2F(Form("hMC_Jet80_CutA_pTeta_R%d_cent%d",radius,i),Form("MC Jet80 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);

    hMinBiasData_noCut_eta_cent[i] = new TH1F(Form("hMinBiasData_noCut_eta_R%d_cent%d",radius,i),Form("MinBiasData no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
    hMinBiasData_CutA_eta_cent[i] = new TH1F(Form("hMinBiasData_CutA_eta_R%d_cent%d",radius,i),Form("MinBiasData CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet55_noCut_eta_cent[i] = new TH1F(Form("hMinBiasData_Jet55_noCut_eta_R%d_cent%d",radius,i),Form("MinBiasData Jet55 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet55_CutA_eta_cent[i] = new TH1F(Form("hMinBiasData_Jet55_CutA_eta_R%d_cent%d",radius,i),Form("MinBiasData Jet55 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet65_noCut_eta_cent[i] = new TH1F(Form("hMinBiasData_Jet65_noCut_eta_R%d_cent%d",radius,i),Form("MinBiasData Jet65 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet65_CutA_eta_cent[i] = new TH1F(Form("hMinBiasData_Jet65_CutA_eta_R%d_cent%d",radius,i),Form("MinBiasData Jet65 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet80_noCut_eta_cent[i] = new TH1F(Form("hMinBiasData_Jet80_noCut_eta_R%d_cent%d",radius,i),Form("MinBiasData Jet80 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet80_CutA_eta_cent[i] = new TH1F(Form("hMinBiasData_Jet80_CutA_eta_R%d_cent%d",radius,i),Form("MinBiasData Jet80 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);

    hMinBiasData_noCut_phi_cent[i] = new TH1F(Form("hMinBiasData_noCut_phi_R%d_cent%d",radius,i),Form("MinBiasData no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
    hMinBiasData_CutA_phi_cent[i] = new TH1F(Form("hMinBiasData_CutA_phi_R%d_cent%d",radius,i),Form("MinBiasData CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet55_noCut_phi_cent[i] = new TH1F(Form("hMinBiasData_Jet55_noCut_phi_R%d_cent%d",radius,i),Form("MinBiasData Jet55 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet55_CutA_phi_cent[i] = new TH1F(Form("hMinBiasData_Jet55_CutA_phi_R%d_cent%d",radius,i),Form("MinBiasData Jet55 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet65_noCut_phi_cent[i] = new TH1F(Form("hMinBiasData_Jet65_noCut_phi_R%d_cent%d",radius,i),Form("MinBiasData Jet65 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet65_CutA_phi_cent[i] = new TH1F(Form("hMinBiasData_Jet65_CutA_phi_R%d_cent%d",radius,i),Form("MinBiasData Jet65 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet80_noCut_phi_cent[i] = new TH1F(Form("hMinBiasData_Jet80_noCut_phi_R%d_cent%d",radius,i),Form("MinBiasData Jet80 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hMinBiasData_Jet80_CutA_phi_cent[i] = new TH1F(Form("hMinBiasData_Jet80_CutA_phi_R%d_cent%d",radius,i),Form("MinBiasData Jet80 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);

    hHydjetMC_noCut_eta_cent[i] = new TH1F(Form("hHydjetMC_noCut_eta_R%d_cent%d",radius,i),Form("HydjetMC no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
    hHydjetMC_CutA_eta_cent[i] = new TH1F(Form("hHydjetMC_CutA_eta_R%d_cent%d",radius,i),Form("HydjetMC CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hHydjetMC_Jet55_noCut_eta_cent[i] = new TH1F(Form("hHydjetMC_Jet55_noCut_eta_R%d_cent%d",radius,i),Form("HydjetMC Jet55 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hHydjetMC_Jet55_CutA_eta_cent[i] = new TH1F(Form("hHydjetMC_Jet55_CutA_eta_R%d_cent%d",radius,i),Form("HydjetMC Jet55 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hHydjetMC_Jet65_noCut_eta_cent[i] = new TH1F(Form("hHydjetMC_Jet65_noCut_eta_R%d_cent%d",radius,i),Form("HydjetMC Jet65 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hHydjetMC_Jet65_CutA_eta_cent[i] = new TH1F(Form("hHydjetMC_Jet65_CutA_eta_R%d_cent%d",radius,i),Form("HydjetMC Jet65 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hHydjetMC_Jet80_noCut_eta_cent[i] = new TH1F(Form("hHydjetMC_Jet80_noCut_eta_R%d_cent%d",radius,i),Form("HydjetMC Jet80 no cuts eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);
   hHydjetMC_Jet80_CutA_eta_cent[i] = new TH1F(Form("hHydjetMC_Jet80_CutA_eta_R%d_cent%d",radius,i),Form("HydjetMC Jet80 CutA eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-2.5,2.5);

    hHydjetMC_noCut_phi_cent[i] = new TH1F(Form("hHydjetMC_noCut_phi_R%d_cent%d",radius,i),Form("HydjetMC no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
    hHydjetMC_CutA_phi_cent[i] = new TH1F(Form("hHydjetMC_CutA_phi_R%d_cent%d",radius,i),Form("HydjetMC CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hHydjetMC_Jet55_noCut_phi_cent[i] = new TH1F(Form("hHydjetMC_Jet55_noCut_phi_R%d_cent%d",radius,i),Form("HydjetMC Jet55 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hHydjetMC_Jet55_CutA_phi_cent[i] = new TH1F(Form("hHydjetMC_Jet55_CutA_phi_R%d_cent%d",radius,i),Form("HydjetMC Jet55 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hHydjetMC_Jet65_noCut_phi_cent[i] = new TH1F(Form("hHydjetMC_Jet65_noCut_phi_R%d_cent%d",radius,i),Form("HydjetMC Jet65 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hHydjetMC_Jet65_CutA_phi_cent[i] = new TH1F(Form("hHydjetMC_Jet65_CutA_phi_R%d_cent%d",radius,i),Form("HydjetMC Jet65 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hHydjetMC_Jet80_noCut_phi_cent[i] = new TH1F(Form("hHydjetMC_Jet80_noCut_phi_R%d_cent%d",radius,i),Form("HydjetMC Jet80 no cuts phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);
   hHydjetMC_Jet80_CutA_phi_cent[i] = new TH1F(Form("hHydjetMC_Jet80_CutA_phi_R%d_cent%d",radius,i),Form("HydjetMC Jet80 CutA phi R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),30,-pi,pi);

   hMinBiasData_Jet55_noCut_JetpTeta[i] = new TH2F(Form("hMinBiasData_Jet55_noCut_pTeta_R%d_cent%d",radius,i),Form("MinBiasData Jet55 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hMinBiasData_Jet55_CutA_JetpTeta[i] = new TH2F(Form("hMinBiasData_Jet55_CutA_pTeta_R%d_cent%d",radius,i),Form("MinBiasData Jet55 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hMinBiasData_Jet65_noCut_JetpTeta[i] = new TH2F(Form("hMinBiasData_Jet65_noCut_pTeta_R%d_cent%d",radius,i),Form("MinBiasData Jet65 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hMinBiasData_Jet65_CutA_JetpTeta[i] = new TH2F(Form("hMinBiasData_Jet65_CutA_pTeta_R%d_cent%d",radius,i),Form("MinBiasData Jet65 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hMinBiasData_Jet80_noCut_JetpTeta[i] = new TH2F(Form("hMinBiasData_Jet80_noCut_pTeta_R%d_cent%d",radius,i),Form("MinBiasData Jet80 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hMinBiasData_Jet80_CutA_JetpTeta[i] = new TH2F(Form("hMinBiasData_Jet80_CutA_pTeta_R%d_cent%d",radius,i),Form("MinBiasData Jet80 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);

   hHydjetMC_Jet55_noCut_JetpTeta[i] = new TH2F(Form("hHydjetMC_Jet55_noCut_pTeta_R%d_cent%d",radius,i),Form("HydjetMC Jet55 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hHydjetMC_Jet55_CutA_JetpTeta[i] = new TH2F(Form("hHydjetMC_Jet55_CutA_pTeta_R%d_cent%d",radius,i),Form("HydjetMC Jet55 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hHydjetMC_Jet65_noCut_JetpTeta[i] = new TH2F(Form("hHydjetMC_Jet65_noCut_pTeta_R%d_cent%d",radius,i),Form("HydjetMC Jet65 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hHydjetMC_Jet65_CutA_JetpTeta[i] = new TH2F(Form("hHydjetMC_Jet65_CutA_pTeta_R%d_cent%d",radius,i),Form("HydjetMC Jet65 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
   hHydjetMC_Jet80_noCut_JetpTeta[i] = new TH2F(Form("hHydjetMC_Jet80_noCut_pTeta_R%d_cent%d",radius,i),Form("HydjetMC Jet80 no cuts pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);   
   hHydjetMC_Jet80_CutA_JetpTeta[i] = new TH2F(Form("hHydjetMC_Jet80_CutA_pTeta_R%d_cent%d",radius,i),Form("HydjetMC Jet80 CutA pT-eta R%d %2.0f - %2.0f cent",radius,5*(float)boundaries_cent[i],5*(float)boundaries_cent[i+1]),100,-2.5,2.5,1000,0,400);
    
   	//TH2F(const char* name, const char* title, Int_t nbinsx, Double_t xlow, Double_t xup, Int_t nbinsy, Double_t ylow, Double_t yup)
   //hpbpb_mcclosure_matrix_HLT[i] = new TH2F(Form("hpbpb_mcclosure_matrix_HLT_R%d_%s_cent%d",radius,etaWidth,i),Form("Matrix for mcclosure refpt jtpt from Jet triggers R%d %s %2.0f - %2.0f cent",radius,etaWidth,5*boundaries_cent[i],5*boundaries_cent[i+1]),400,0,400,400,0,400);
   //prev. line: I'm an example
   
   //define canvases
   cmatrix_HTL[i] = new TCanvas;
   cPbPb_pteta_noCut[i] = new TCanvas(Form("hData_noCut_pTeta_R%d_cent%d",radius,i),Form("hData_noCut_pTeta_R%d_cent%d",radius,i),1500,600);
   cPbPb_pteta_CutA[i] = new TCanvas(Form("hData_CutA_pTeta_R%d_cent%d",radius,i),Form("hData_CutA_pTeta_R%d_cent%d",radius,i),1500,600);
   cMC_pteta_noCut[i] = new TCanvas(Form("hMC_noCut_pTeta_R%d_cent%d",radius,i),Form("hMC_noCut_pTeta_R%d_cent%d",radius,i),1500,600);
   cMC_pteta_CutA[i] = new TCanvas(Form("hMC_CutA_pTeta_R%d_cent%d",radius,i),Form("hMC_CutA_pTeta_R%d_cent%d",radius,i),1500,600);
	
  }// centrality bin loop
  
  
  // 1 - Data, 2 - MC, 3 - MinBias data, 4 - MinBias MC
  Float_t pfpt_1, pfpt_2;
  Float_t pfrefpt_2;
  Float_t calopt_1, calopt_2;
  Float_t eMax_1, eMax_2;
  Float_t chMax_1, chMax_2;
  Float_t chSum_1, chSum_2;
  Float_t phSum_1, phSum_2;
  Float_t neSum_1, neSum_2;
  Float_t muSum_1, muSum_2;
  Float_t pfeta_1, pfeta_2;
  Float_t pfphi_1, pfphi_2;
  Int_t jet55_1, jet65_1, jet80_1;
  Int_t jet55_p_1, jet65_p_1, jet80_p_1;
  Int_t jet55_2, jet65_2, jet80_2;
  Int_t jet55_p_2, jet65_p_2, jet80_p_2;
  Float_t weight;
  Int_t subid_2;
  Int_t hiBin_1, hiBin_2;

  Data_matched->SetBranchAddress("calopt",&calopt_1);
  Data_matched->SetBranchAddress("pfpt",&pfpt_1);
  Data_matched->SetBranchAddress("eMax",&eMax_1);
  Data_matched->SetBranchAddress("chMax",&chMax_1);
  Data_matched->SetBranchAddress("chSum",&chSum_1);
  Data_matched->SetBranchAddress("phSum",&phSum_1);
  Data_matched->SetBranchAddress("neSum",&neSum_1);
  Data_matched->SetBranchAddress("muSum",&muSum_1);
  Data_matched->SetBranchAddress("hiBin",&hiBin_1);
  Data_matched->SetBranchAddress("jet55",&jet55_1);
  Data_matched->SetBranchAddress("jet65",&jet65_1);
  Data_matched->SetBranchAddress("jet80",&jet80_1);
  Data_matched->SetBranchAddress("jet55_prescl",&jet55_p_1);
  Data_matched->SetBranchAddress("jet65_prescl",&jet65_p_1);
  Data_matched->SetBranchAddress("jet80_prescl",&jet80_p_1);
  Data_matched->SetBranchAddress("pfeta",&pfeta_1);
  Data_matched->SetBranchAddress("pfphi",&pfphi_1);
  
  Data_unmatched->SetBranchAddress("pfpt",&pfpt_1);
  Data_unmatched->SetBranchAddress("eMax",&eMax_1);
  Data_unmatched->SetBranchAddress("chMax",&chMax_1);
  Data_unmatched->SetBranchAddress("chSum",&chSum_1);
  Data_unmatched->SetBranchAddress("phSum",&phSum_1);
  Data_unmatched->SetBranchAddress("neSum",&neSum_1);
  Data_unmatched->SetBranchAddress("muSum",&muSum_1);
  Data_unmatched->SetBranchAddress("hiBin",&hiBin_1);
  Data_unmatched->SetBranchAddress("jet55",&jet55_1);
  Data_unmatched->SetBranchAddress("jet65",&jet65_1);
  Data_unmatched->SetBranchAddress("jet80",&jet80_1);
  Data_unmatched->SetBranchAddress("jet55_prescl",&jet55_p_1);
  Data_unmatched->SetBranchAddress("jet65_prescl",&jet65_p_1);
  Data_unmatched->SetBranchAddress("jet80_prescl",&jet80_p_1);
  Data_unmatched->SetBranchAddress("pfeta",&pfeta_1);
  Data_unmatched->SetBranchAddress("pfphi",&pfphi_1);
  
  MC_matched->SetBranchAddress("calopt",&calopt_2);
  MC_matched->SetBranchAddress("pfpt",&pfpt_2);
  MC_matched->SetBranchAddress("eMax",&eMax_2);
  MC_matched->SetBranchAddress("chMax",&chMax_2);
  MC_matched->SetBranchAddress("chSum",&chSum_2);
  MC_matched->SetBranchAddress("phSum",&phSum_2);
  MC_matched->SetBranchAddress("neSum",&neSum_2);
  MC_matched->SetBranchAddress("muSum",&muSum_2);
  MC_matched->SetBranchAddress("hiBin",&hiBin_2);
  MC_matched->SetBranchAddress("pfrefpt",&pfrefpt_2);
  MC_matched->SetBranchAddress("jet55",&jet55_2);
  MC_matched->SetBranchAddress("jet65",&jet65_2);
  MC_matched->SetBranchAddress("jet80",&jet80_2);
  MC_matched->SetBranchAddress("weight", &weight);
  MC_matched->SetBranchAddress("subid", &subid_2);
  MC_matched->SetBranchAddress("pfeta", &pfeta_2);
  MC_matched->SetBranchAddress("pfphi", &pfphi_2);
  MC_matched->SetBranchAddress("pfrefpt", &pfrefpt_2);
  MC_matched->SetBranchAddress("jet55_prescl",&jet55_p_2);

  MC_unmatched->SetBranchAddress("pfpt",&pfpt_2);
  MC_unmatched->SetBranchAddress("eMax",&eMax_2);
  MC_unmatched->SetBranchAddress("chMax",&chMax_2);
  MC_unmatched->SetBranchAddress("chSum",&chSum_2);
  MC_unmatched->SetBranchAddress("phSum",&phSum_2);
  MC_unmatched->SetBranchAddress("neSum",&neSum_2);
  MC_unmatched->SetBranchAddress("muSum",&muSum_2);
  MC_unmatched->SetBranchAddress("hiBin",&hiBin_2);
  MC_unmatched->SetBranchAddress("pfrefpt",&pfrefpt_2);
  MC_unmatched->SetBranchAddress("jet55",&jet55_2);
  MC_unmatched->SetBranchAddress("jet65",&jet65_2);
  MC_unmatched->SetBranchAddress("jet80",&jet80_2);
  MC_unmatched->SetBranchAddress("weight", & weight);
  MC_unmatched->SetBranchAddress("subid", &subid_2);
  MC_unmatched->SetBranchAddress("pfeta", &pfeta_2);
  MC_unmatched->SetBranchAddress("pfphi", &pfphi_2);
  MC_unmatched->SetBranchAddress("pfrefpt", &pfrefpt_2);
  MC_unmatched->SetBranchAddress("jet55_prescl",&jet55_p_2);
  
// 3 - MinBias data, 4 - MinBias MC
  Float_t pfpt_3, pfpt_4;
  Float_t pfrefpt_4;
  Float_t calopt_3, calopt_4;
  Float_t eMax_3, eMax_4;
  Float_t chMax_3, chMax_4;
  Float_t chSum_3, chSum_4;
  Float_t phSum_3, phSum_4;
  Float_t neSum_3, neSum_4;
  Float_t muSum_3, muSum_4;
  Float_t pfeta_3, pfeta_4;
  Float_t pfphi_3, pfphi_4;
  Int_t jet55_3, jet65_3, jet80_3;
  Int_t jet55_p_3, jet65_p_3, jet80_p_3;
  Int_t jet55_4, jet65_4, jet80_4;
  Int_t jet55_p_4, jet65_p_4, jet80_p_4;
  Int_t subid_4;
  Int_t hiBin_3, hiBin_4;
    Int_t jetMB_3, jetMB_p_3;
    Int_t jetMB_4, jetMB_p_4;


  MinBias_matched->SetBranchAddress("calopt",&calopt_3);
  MinBias_matched->SetBranchAddress("pfpt",&pfpt_3);
  MinBias_matched->SetBranchAddress("eMax",&eMax_3);
  MinBias_matched->SetBranchAddress("chMax",&chMax_3);
  MinBias_matched->SetBranchAddress("chSum",&chSum_3);
  MinBias_matched->SetBranchAddress("phSum",&phSum_3);
  MinBias_matched->SetBranchAddress("neSum",&neSum_3);
  MinBias_matched->SetBranchAddress("muSum",&muSum_3);
  MinBias_matched->SetBranchAddress("hiBin",&hiBin_3);
  MinBias_matched->SetBranchAddress("jet55",&jet55_3);
  MinBias_matched->SetBranchAddress("jet65",&jet65_3);
  MinBias_matched->SetBranchAddress("jet80",&jet80_3);
  MinBias_matched->SetBranchAddress("jet55_prescl",&jet55_p_3);
  MinBias_matched->SetBranchAddress("jet65_prescl",&jet65_p_3);
  MinBias_matched->SetBranchAddress("jet80_prescl",&jet80_p_3);
  MinBias_matched->SetBranchAddress("jetMB",&jetMB_3);
  MinBias_matched->SetBranchAddress("jetMB_prescl",&jetMB_p_3);  
  MinBias_matched->SetBranchAddress("pfeta",&pfeta_3);
  MinBias_matched->SetBranchAddress("pfphi",&pfphi_3);
  
  MinBias_unmatched->SetBranchAddress("pfpt",&pfpt_3);
  MinBias_unmatched->SetBranchAddress("eMax",&eMax_3);
  MinBias_unmatched->SetBranchAddress("chMax",&chMax_3);
  MinBias_unmatched->SetBranchAddress("chSum",&chSum_3);
  MinBias_unmatched->SetBranchAddress("phSum",&phSum_3);
  MinBias_unmatched->SetBranchAddress("neSum",&neSum_3);
  MinBias_unmatched->SetBranchAddress("muSum",&muSum_3);
  MinBias_unmatched->SetBranchAddress("hiBin",&hiBin_3);
  MinBias_unmatched->SetBranchAddress("jet55",&jet55_3);
  MinBias_unmatched->SetBranchAddress("jet65",&jet65_3);
  MinBias_unmatched->SetBranchAddress("jet80",&jet80_3);
  MinBias_unmatched->SetBranchAddress("jet55_prescl",&jet55_p_3);
  MinBias_unmatched->SetBranchAddress("jet65_prescl",&jet65_p_3);
  MinBias_unmatched->SetBranchAddress("jet80_prescl",&jet80_p_3);
  MinBias_unmatched->SetBranchAddress("jetMB",&jetMB_3);
  MinBias_unmatched->SetBranchAddress("jetMB_prescl",&jetMB_p_3);    
  MinBias_unmatched->SetBranchAddress("pfeta",&pfeta_3);
  MinBias_unmatched->SetBranchAddress("pfphi",&pfphi_3);
  
  Hydjet_matched->SetBranchAddress("calopt",&calopt_4);
  Hydjet_matched->SetBranchAddress("pfpt",&pfpt_4);
  Hydjet_matched->SetBranchAddress("eMax",&eMax_4);
  Hydjet_matched->SetBranchAddress("chMax",&chMax_4);
  Hydjet_matched->SetBranchAddress("chSum",&chSum_4);
  Hydjet_matched->SetBranchAddress("phSum",&phSum_4);
  Hydjet_matched->SetBranchAddress("neSum",&neSum_4);
  Hydjet_matched->SetBranchAddress("muSum",&muSum_4);
  Hydjet_matched->SetBranchAddress("hiBin",&hiBin_4);
  Hydjet_matched->SetBranchAddress("pfrefpt",&pfrefpt_4);
  Hydjet_matched->SetBranchAddress("jet55",&jet55_4);
  Hydjet_matched->SetBranchAddress("jet65",&jet65_4);
  Hydjet_matched->SetBranchAddress("jet80",&jet80_4);
  Hydjet_matched->SetBranchAddress("jetMB",&jetMB_4);
  Hydjet_matched->SetBranchAddress("jetMB_prescl",&jetMB_p_4);    
//  Hydjet_matched->SetBranchAddress("weight", &weight);
  Hydjet_matched->SetBranchAddress("subid", &subid_4);
  Hydjet_matched->SetBranchAddress("pfeta", &pfeta_4);
  Hydjet_matched->SetBranchAddress("pfphi", &pfphi_4);
  Hydjet_matched->SetBranchAddress("pfrefpt", &pfrefpt_4);
  Hydjet_matched->SetBranchAddress("jet55_prescl",&jet55_p_4);

  Hydjet_unmatched->SetBranchAddress("pfpt",&pfpt_4);
  Hydjet_unmatched->SetBranchAddress("eMax",&eMax_4);
  Hydjet_unmatched->SetBranchAddress("chMax",&chMax_4);
  Hydjet_unmatched->SetBranchAddress("chSum",&chSum_4);
  Hydjet_unmatched->SetBranchAddress("phSum",&phSum_4);
  Hydjet_unmatched->SetBranchAddress("neSum",&neSum_4);
  Hydjet_unmatched->SetBranchAddress("muSum",&muSum_4);
  Hydjet_unmatched->SetBranchAddress("hiBin",&hiBin_4);
  Hydjet_unmatched->SetBranchAddress("pfrefpt",&pfrefpt_4);
  Hydjet_unmatched->SetBranchAddress("jet55",&jet55_4);
  Hydjet_unmatched->SetBranchAddress("jet65",&jet65_4);
  Hydjet_unmatched->SetBranchAddress("jet80",&jet80_4);
  Hydjet_unmatched->SetBranchAddress("jetMB",&jetMB_4);
  Hydjet_unmatched->SetBranchAddress("jetMB_prescl",&jetMB_p_4);   
//  Hydjet_unmatched->SetBranchAddress("weight", & weight);
  Hydjet_unmatched->SetBranchAddress("subid", &subid_4);
  Hydjet_unmatched->SetBranchAddress("pfeta", &pfeta_4);
  Hydjet_unmatched->SetBranchAddress("pfphi", &pfphi_4);
  Hydjet_unmatched->SetBranchAddress("pfrefpt", &pfrefpt_4);
  Hydjet_unmatched->SetBranchAddress("jet55_prescl",&jet55_p_4);

  
  // data matched loop
  long entries = MinBias_matched->GetEntries();
  //entries = 1000;
  cout<<"matched Data MinBias ntuple "<<endl;
  
  for(long nentry = 0; nentry < entries; ++nentry ){

    if(nentry%10000 == 0) cout<<" nentry = "<<nentry<<endl;
    MinBias_matched->GetEntry(nentry);

    Int_t cBin = findBin(hiBin_3);
    if(cBin == -1 || cBin >= nbins_cent) continue; //this discriminates based on centrality
    
    Float_t Sumcand = chSum_3 + phSum_3 + neSum_3 + muSum_3;

if(pfpt_3>=ptCutData){
   if(cBin==0 && pfpt_3<ptCutCentralData) { continue; } // cut data at 100 GeV for 0-5% central
   
   hData_noCut_eta->Fill(pfeta_3); //nocut: no prescale
   hData_noCut_phi->Fill(pfphi_3);
   hData_noCut_eta_cent[cBin]->Fill(pfeta_3);
   hData_noCut_phi_cent[cBin]->Fill(pfphi_3);
   hData_noCut_phi_eta->Fill(pfeta_3,pfphi_3);
    if(calopt_3/pfpt_3 > 0.5 && calopt_3/pfpt_3 <= 0.85 && eMax_3/Sumcand < ((Float_t)18/7 *(Float_t)calopt_3/pfpt_3 - (Float_t)9/7)) {
   hData_CutA_eta->Fill(pfeta_3);
   hData_CutA_phi->Fill(pfphi_3);
   hData_CutA_eta_cent[cBin]->Fill(pfeta_3);
   hData_CutA_phi_cent[cBin]->Fill(pfphi_3);
   hData_CutA_phi_eta->Fill(pfeta_3,pfphi_3);
    }
    if(calopt_3/pfpt_3 > 0.85) {
       hData_CutA_eta->Fill(pfeta_3);
   hData_CutA_phi->Fill(pfphi_3);
   hData_CutA_eta_cent[cBin]->Fill(pfeta_3);
   hData_CutA_phi_cent[cBin]->Fill(pfphi_3);
      hData_CutA_phi_eta->Fill(pfeta_3,pfphi_3);   
    }   
    if(calopt_3/pfpt_3 <= 0.5 && eMax_3/Sumcand < 0.05) {
       hData_CutA_eta->Fill(pfeta_3);
   hData_CutA_phi->Fill(pfphi_3);
   hData_CutA_eta_cent[cBin]->Fill(pfeta_3);
   hData_CutA_phi_cent[cBin]->Fill(pfphi_3);
      hData_CutA_phi_eta->Fill(pfeta_3,pfphi_3);
    }
    if(jet55_3 == 1 && jet65_3 == 0 && jet80_3 == 0 ) { //jet 55 matched
      
      hData_Jet55_noCut->Fill(pfpt_3, jet55_p_3);
	  hData_Jet55_noCut_eta->Fill(pfeta_3,jet55_p_3);
	  hData_Jet55_noCut_phi->Fill(pfphi_3,jet55_p_3);
 	  hData_Jet55_noCut_eta_cent[cBin]->Fill(pfeta_3,jet55_p_3);
	  hData_Jet55_noCut_phi_cent[cBin]->Fill(pfphi_3,jet55_p_3); 
      hData_Jet55_noCut_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3,jet55_p_3); //fill pteta
	  
      if(calopt_3/pfpt_3 > 0.5 && calopt_3/pfpt_3 <= 0.85 && eMax_3/Sumcand < ((Float_t)18/7 *(Float_t)calopt_3/pfpt_3 - (Float_t)9/7)) {
		  hData_Jet55_CutA->Fill(pfpt_3, jet55_p_3);
	  hData_Jet55_CutA_eta->Fill(pfeta_3,jet55_p_3);
	  hData_Jet55_CutA_phi->Fill(pfphi_3,jet55_p_3);
 	  hData_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_3,jet55_p_3);
	  hData_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_3,jet55_p_3); 
	  hpbpb_TrgObj55[cBin]->Fill(pfpt_3, jet55_p_3);	  		  
	  hData_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3,jet55_p_3);
	  }
      if(calopt_3/pfpt_3 > 0.85) {
		  hData_Jet55_CutA->Fill(pfpt_3, jet55_p_3);
	  hData_Jet55_CutA_eta->Fill(pfeta_3);
	  hData_Jet55_CutA_phi->Fill(pfphi_3);
 	  hData_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_3);
	  hData_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_3); 
	  		  hpbpb_TrgObj55[cBin]->Fill(pfpt_3, jet55_p_3);
	  hData_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3,jet55_p_3);
	  }
      if(calopt_3/pfpt_3 <= 0.5 && eMax_3/Sumcand < 0.05) {
		  hData_Jet55_CutA->Fill(pfpt_3, jet55_p_3);
	  hData_Jet55_CutA_eta->Fill(pfeta_3);
	  hData_Jet55_CutA_phi->Fill(pfphi_3);
 	  hData_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_3);
	  hData_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_3); 
	  		  hpbpb_TrgObj55[cBin]->Fill(pfpt_3, jet55_p_3);
	  hData_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3,jet55_p_3);
	  }

      if(calopt_3/pfpt_3 <= 0.5 && eMax_3/Sumcand >= 0.05) {
        hData_Jet55_CutA_rej->Fill(pfpt_3, jet55_p_3);
      }
      if(calopt_3/pfpt_3 > 0.5 && calopt_3/pfpt_3 <= 0.85 && eMax_3/Sumcand >= ((Float_t)18/7 *(Float_t)calopt_3/pfpt_3 - (Float_t)9/7)) {
        hData_Jet55_CutA_rej->Fill(pfpt_3, jet55_p_3);
      }
    }
    
    if(jet65_3 == 1 && jet80_3 == 0 ) { //Jet 65 matched
      
      hData_Jet65_noCut->Fill(pfpt_3);
      hData_Jet65_noCut_eta->Fill(pfeta_3);
      hData_Jet65_noCut_phi->Fill(pfphi_3);
      hData_Jet65_noCut_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet65_noCut_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet65_noCut_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
      	  
      if(calopt_3/pfpt_3 > 0.5 && calopt_3/pfpt_3 <= 0.85 && eMax_3/Sumcand < ((Float_t)18/7 *(Float_t)calopt_3/pfpt_3 - (Float_t)9/7)) {
	  hData_Jet65_CutA->Fill(pfpt_3);
	  hData_Jet65_CutA_eta->Fill(pfeta_3);
	hpbpb_TrgObj65[cBin]->Fill(pfpt_3);
      hData_Jet65_CutA_eta->Fill(pfeta_3);
      hData_Jet65_CutA_phi->Fill(pfphi_3);
      hData_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
      
	  if(calopt_3/pfpt_3 > 0.85) {
	  hData_Jet65_CutA->Fill(pfpt_3);
	  hData_Jet65_CutA_eta->Fill(pfeta_3);
	hpbpb_TrgObj65[cBin]->Fill(pfpt_3);
      hData_Jet65_CutA_eta->Fill(pfeta_3);
      hData_Jet65_CutA_phi->Fill(pfphi_3);
      hData_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
	  
      if(calopt_3/pfpt_3 <= 0.5 && eMax_3/Sumcand < 0.05) {
	  hData_Jet65_CutA->Fill(pfpt_3);
	  hData_Jet65_CutA_eta->Fill(pfeta_3);
	hpbpb_TrgObj65[cBin]->Fill(pfpt_3);
      hData_Jet65_CutA_eta->Fill(pfeta_3);
      hData_Jet65_CutA_phi->Fill(pfphi_3);
      hData_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
	  
      if(calopt_3/pfpt_3 <= 0.5 && eMax_3/Sumcand >= 0.05) {
         hData_Jet65_CutA_rej->Fill(pfpt_3);
      }
      if(calopt_3/pfpt_3 > 0.5 && calopt_3/pfpt_3 <= 0.85 && eMax_3/Sumcand >= ((Float_t)18/7 *(Float_t)calopt_3/pfpt_3 - (Float_t)9/7)) {
        hData_Jet65_CutA_rej->Fill(pfpt_3);
      }
    }

    if(jet80_3 == 1) {  //jet 80 matched
    
      hData_Jet80_noCut->Fill(pfpt_3);
      hData_Jet80_noCut_eta->Fill(pfeta_3);
      hData_Jet80_noCut_phi->Fill(pfphi_3);
      hData_Jet80_noCut_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet80_noCut_phi_cent[cBin]->Fill(pfphi_3);
      hData_Jet80_noCut_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
			
      if(calopt_3/pfpt_3 > 0.5 && calopt_3/pfpt_3 <= 0.85 && eMax_3/Sumcand < ((Float_t)18/7 *(Float_t)calopt_3/pfpt_3 - (Float_t)9/7)) {
		  hData_Jet80_CutA->Fill(pfpt_3);
		  hData_Jet80_CutA_eta->Fill(pfeta_3);
	hpbpb_TrgObj80[cBin]->Fill(pfpt_3);
      hData_Jet80_CutA_eta->Fill(pfeta_3);
      hData_Jet80_CutA_phi->Fill(pfphi_3);
      hData_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
      
	  if(calopt_3/pfpt_3 > 0.85) {
		  hData_Jet80_CutA->Fill(pfpt_3);
		  hData_Jet80_CutA_eta->Fill(pfeta_3);
	hpbpb_TrgObj80[cBin]->Fill(pfpt_3);
      hData_Jet80_CutA_eta->Fill(pfeta_3);
      hData_Jet80_CutA_phi->Fill(pfphi_3);
      hData_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
      
	  if(calopt_3/pfpt_3 <= 0.5 && eMax_3/Sumcand < 0.05) {
		  hData_Jet80_CutA->Fill(pfpt_3);
		  hData_Jet80_CutA_eta->Fill(pfeta_3);
	hpbpb_TrgObj80[cBin]->Fill(pfpt_3);
      hData_Jet80_CutA_eta->Fill(pfeta_3);
      hData_Jet80_CutA_phi->Fill(pfphi_3);
      hData_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
		  
      if(calopt_3/pfpt_3 <= 0.5 && eMax_3/Sumcand >= 0.05) hData_Jet80_CutA_rej->Fill(pfpt_3);
      if(calopt_3/pfpt_3 > 0.5 && calopt_3/pfpt_3 <= 0.85 && eMax_3/Sumcand >= ((Float_t)18/7 *(Float_t)calopt_3/pfpt_3 - (Float_t)9/7)) hData_Jet80_CutA_rej->Fill(pfpt_3);
      
    }
	} //end ptCutData
  }// data ntuple loop

  // data unmatched loop:
  entries = MinBias_unmatched->GetEntries();
  //entries = 1000;
  cout<<"Unmatched Data ntuple "<<endl;
  for(long nentry = 0; nentry < entries; ++nentry ){

    if(nentry%10000 == 0) cout<<" nentry = "<<nentry<<endl;
    MinBias_unmatched->GetEntry(nentry);
    Int_t cBin = findBin(hiBin_3);
    if(cBin == -1 || cBin >= nbins_cent) continue;
    
 if(pfpt_3>=ptCutData){
   if(cBin==0 && pfpt_3<ptCutCentralData) { continue; } // cut data at 100 GeV for 0-5% central    
   
    Float_t Sumcand = chSum_3 + phSum_3 + neSum_3 + muSum_3;
   hData_noCut_eta->Fill(pfeta_3);
   hData_noCut_phi->Fill(pfphi_3);
   hData_noCut_eta_cent[cBin]->Fill(pfeta_3);
   hData_noCut_phi_cent[cBin]->Fill(pfphi_3);
   hData_noCut_phi_eta->Fill(pfeta_3,pfphi_3);
    if(eMax_3/Sumcand < 0.05 ){
   hData_CutA_eta->Fill(pfeta_3);
   hData_CutA_phi->Fill(pfphi_3);
   hData_CutA_eta_cent[cBin]->Fill(pfeta_3);
   hData_CutA_phi_cent[cBin]->Fill(pfphi_3); 
   hData_CutA_phi_eta->Fill(pfeta_3,pfphi_3);
    }   
    if(jet55_3 == 1 && jet65_3 == 0 && jet80_3 == 0 ) {  //jet55 unmatched
    
      hData_unmatched_Jet55_noCut->Fill(pfpt_3, jet55_p_3);
	  hData_Jet55_noCut_eta->Fill(pfeta_3, jet55_p_3); //added prescale here Apr 9 2015
	  hData_Jet55_noCut_phi->Fill(pfphi_3, jet55_p_3); //added prescale here
 	  hData_Jet55_noCut_eta_cent[cBin]->Fill(pfeta_3, jet55_p_3); //added prescale here
	  hData_Jet55_noCut_phi_cent[cBin]->Fill(pfphi_3, jet55_p_3); //added prescale here
	  hData_Jet55_noCut_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3,jet55_p_3);
	  
      if(eMax_3/Sumcand < 0.05 ){
        hpbpb_TrgObj55[cBin]->Fill(pfpt_3,jet55_p_3);
	    hData_unmatched_Jet55_CutA->Fill(pfpt_3, jet55_p_3);
	    hData_Jet55_CutA_eta->Fill(pfeta_3,jet55_p_3);
	  hData_Jet55_CutA_phi->Fill(pfphi_3,jet55_p_3);
 	  hData_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_3,jet55_p_3);
	  hData_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_3,jet55_p_3); 
	  hData_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3,jet55_p_3);
	  }
      else hData_unmatched_Jet55_CutA_rej->Fill(pfpt_3, jet55_p_3);
      
    }

    if(jet65_3 == 1 && jet80_3 == 0 ) {

      hData_unmatched_Jet65_noCut->Fill(pfpt_3);
      hData_Jet65_noCut_eta->Fill(pfeta_3);
      hData_Jet65_noCut_phi->Fill(pfphi_3);
      hData_Jet65_noCut_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet65_noCut_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet65_noCut_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
      if(eMax_3/Sumcand < 0.05  ){
         hpbpb_TrgObj65[cBin]->Fill(pfpt_3);
	     hData_unmatched_Jet65_CutA->Fill(pfpt_3);
      hData_Jet65_CutA_eta->Fill(pfeta_3);
      hData_Jet65_CutA_phi->Fill(pfphi_3);
      hData_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_3);
	  hData_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
      else hData_unmatched_Jet65_CutA_rej->Fill(pfpt_3);
      
    }

    if(jet80_3 == 1) {
    
      hData_unmatched_Jet80_noCut->Fill(pfpt_3);
      hData_Jet80_noCut_eta->Fill(pfeta_3);
      hData_Jet80_noCut_phi->Fill(pfphi_3);
      hData_Jet80_noCut_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet80_noCut_phi_cent[cBin]->Fill(pfphi_3);
      hData_Jet80_noCut_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
      if(eMax_3/Sumcand < 0.05  ){
          hpbpb_TrgObj80[cBin]->Fill(pfpt_3);
	      hData_unmatched_Jet80_CutA->Fill(pfpt_3);
      hData_Jet80_CutA_eta->Fill(pfeta_3);
      hData_Jet80_CutA_phi->Fill(pfphi_3);
      hData_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_3);
      hData_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_3);	      
	  hData_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_3,pfpt_3);
	  }
      else hData_unmatched_Jet80_CutA_rej->Fill(pfpt_3);
      
    }
	} //end check pT
  }// data ntuple loop

  entries = Hydjet_matched->GetEntries();
  //entries = 1000;
  // MC loop
  cout<<" looking at matched Hydjet MC ntuple "<<endl;
  for(long nentry = 0; nentry < entries; ++nentry){

    if(nentry%10000 == 0) cout<<" nentry = "<<nentry<<endl;
    Hydjet_matched->GetEntry(nentry);
    
    Float_t Sumcand = chSum_4 + phSum_4 + neSum_4 + muSum_4;
    if(subid_4 != 0) continue;

    Int_t cBin = findBin(hiBin_4);
    if(cBin == -1 || cBin >= nbins_cent) continue;
    
 if(pfpt_4>=ptCutMC){
   if(cBin==0 && pfpt_4<ptCutCentralMC) { continue; } // cut data at 100 GeV for 0-5% central   
   
   hMC_noCut_eta->Fill(pfeta_4,weight);
   hMC_noCut_phi->Fill(pfphi_4,weight);
   hMC_noCut_eta_cent[cBin]->Fill(pfeta_4,weight);
   hMC_noCut_phi_cent[cBin]->Fill(pfphi_4,weight);
   hMC_noCut_phi_eta->Fill(pfeta_4,pfphi_4,weight);
  
    if(calopt_4/pfpt_4 > 0.5 && calopt_4/pfpt_4 <= 0.85 && eMax_4/Sumcand < ((Float_t)18/7 *(Float_t)calopt_4/pfpt_4 - (Float_t)9/7)) {
   hMC_CutA_eta->Fill(pfeta_4,weight);
   hMC_CutA_phi->Fill(pfphi_4,weight);
   hMC_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
   hMC_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_CutA_phi_eta->Fill(pfeta_4,pfphi_4,weight);
   
    }
    if(calopt_4/pfpt_4 > 0.85) {
       hMC_CutA_eta->Fill(pfeta_4,weight);
   hMC_CutA_phi->Fill(pfphi_4,weight);
   hMC_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
   hMC_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
   hMC_CutA_phi_eta->Fill(pfeta_4,pfphi_4,weight);
   
    }   
    if(calopt_4/pfpt_4 <= 0.5 && eMax_4/Sumcand < 0.05) {
       hMC_CutA_eta->Fill(pfeta_4,weight);
   hMC_CutA_phi->Fill(pfphi_4,weight);
   hMC_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
   hMC_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
   hMC_CutA_phi_eta->Fill(pfeta_4,pfphi_4,weight);
   
    }    
    if(jet55_4 == 1 && jet65_4==0 && jet80_4 == 0){
      
      hMC_Jet55_noCut->Fill(pfrefpt_4, weight);
	  hMC_Jet55_noCut_eta->Fill(pfeta_4, weight);
	  hMC_Jet55_noCut_phi->Fill(pfphi_4, weight);
 	  hMC_Jet55_noCut_eta_cent[cBin]->Fill(pfeta_4, weight);
	  hMC_Jet55_noCut_phi_cent[cBin]->Fill(pfphi_4, weight);
	  hMC_Jet55_noCut_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
      if(calopt_4/pfpt_4 > 0.5 && calopt_4/pfpt_4 <= 0.85 && eMax_4/Sumcand < ((Float_t)18/7 *(Float_t)calopt_4/pfpt_4 - (Float_t)9/7)){
	hMC_Jet55_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet55_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hMC_Jet55_CutA_eta->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi->Fill(pfphi_4, weight);
 	  hMC_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_4, weight); 
	  hMC_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet55_data[cBin]->Fill(pfpt_4, weight);
	}
	
      }
      if(calopt_4/pfpt_4 > 0.85) {
	hMC_Jet55_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet55_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hMC_Jet55_CutA_eta->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi->Fill(pfphi_4, weight);
 	  hMC_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_4, weight); 
	  hMC_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet55_data[cBin]->Fill(pfpt_4, weight);
	}
	
      }
      if(calopt_4/pfpt_4 <= 0.5 && eMax_4/Sumcand < 0.05) {
	hMC_Jet55_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet55_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hMC_Jet55_CutA_eta->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi->Fill(pfphi_4, weight);
 	  hMC_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_4, weight); 
	  hMC_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet55_data[cBin]->Fill(pfrefpt_4, weight);
	}
	
      }
      if(calopt_4/pfpt_4 <= 0.5 && eMax_4/Sumcand >= 0.05) hMC_Jet55_CutA_rej->Fill(pfrefpt_4, weight);
      if(calopt_4/pfpt_4 > 0.5 && calopt_4/pfpt_4 <= 0.85 && eMax_4/Sumcand >= ((Float_t)18/7 *(Float_t)calopt_4/pfpt_4 - (Float_t)9/7)) hMC_Jet55_CutA_rej->Fill(pfrefpt_4, weight);
      
    }
    
    if(jet65_4 == 1 && jet80_4 == 0){
      

      hMC_Jet65_noCut->Fill(pfrefpt_4, weight);
      hMC_Jet65_noCut_eta->Fill(pfeta_4,weight);
      hMC_Jet65_noCut_phi->Fill(pfphi_4,weight);
      hMC_Jet65_noCut_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet65_noCut_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet65_noCut_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
      if(calopt_4/pfpt_4 > 0.5 && calopt_4/pfpt_4 <= 0.85 && eMax_4/Sumcand < ((Float_t)18/7 *(Float_t)calopt_4/pfpt_4 - (Float_t)9/7)) {
	hMC_Jet65_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet65_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet65_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet65_data[cBin]->Fill(pfpt_4, weight);
	}
	
      }
      if(calopt_4/pfpt_4 > 0.85) {
	hMC_Jet65_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet65_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet65_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet65_data[cBin]->Fill(pfpt_4, weight);
	}
      }
      if(calopt_4/pfpt_4 <= 0.5 && eMax_4/Sumcand < 0.05) {
	hMC_Jet65_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet65_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet65_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet65_data[cBin]->Fill(pfpt_4, weight);
	}
      }

      if(calopt_4/pfpt_4 <= 0.5 && eMax_4/Sumcand >= 0.05) hMC_Jet65_CutA_rej->Fill(pfrefpt_4, weight);
      if(calopt_4/pfpt_4 > 0.5 && calopt_4/pfpt_4 <= 0.85 && eMax_4/Sumcand >= ((Float_t)18/7 *(Float_t)calopt_4/pfpt_4 - (Float_t)9/7)) hMC_Jet65_CutA_rej->Fill(pfrefpt_4, weight);

    }

    
    if(jet80_4 == 1){

      hMC_Jet80_noCut->Fill(pfrefpt_4, weight);
      hMC_Jet80_noCut_eta->Fill(pfeta_4,weight);
      hMC_Jet80_noCut_phi->Fill(pfphi_4,weight);
      hMC_Jet80_noCut_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet80_noCut_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet80_noCut_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
      if(calopt_4/pfpt_4 > 0.5 && calopt_4/pfpt_4 <= 0.85 && eMax_4/Sumcand < ((Float_t)18/7 *(Float_t)calopt_4/pfpt_4 - (Float_t)9/7)) {
	hMC_Jet80_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet80_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet80_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet80_data[cBin]->Fill(pfpt_4, weight);
	}
      }
      if(calopt_4/pfpt_4 > 0.85) {
	hMC_Jet80_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet80_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet80_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet80_data[cBin]->Fill(pfpt_4, weight);
	}
      }
      if(calopt_4/pfpt_4 <= 0.5 && eMax_4/Sumcand < 0.05) {
	hMC_Jet80_CutA->Fill(pfrefpt_4, weight);
	hpbpb_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet80_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet80_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet80_data[cBin]->Fill(pfpt_4, weight);
	}
      }
      
      if(calopt_4/pfpt_4 <= 0.5 && eMax_4/Sumcand >= 0.05) hMC_Jet80_CutA_rej->Fill(pfrefpt_4, weight);
      if(calopt_4/pfpt_4 > 0.5 && calopt_4/pfpt_4 <= 0.85 && eMax_4/Sumcand >= ((Float_t)18/7 *(Float_t)calopt_4/pfpt_4 - (Float_t)9/7)) hMC_Jet80_CutA_rej->Fill(pfrefpt_4, weight);

    }
 } //loop end check pT
    
  }// mc ntuple loop


  entries = MC_unmatched->GetEntries();
  //entries = 1000;
  // MC loop
  cout<<" looking at unmatched MC ntuple"<<endl;
  for(long nentry = 0; nentry < entries; ++nentry){

    if(nentry%10000 == 0) cout<<" nentry = "<<nentry<<endl;
    MC_unmatched->GetEntry(nentry);

    if(subid_4 != 0) continue;

    Int_t cBin = findBin(hiBin_4);
    if(cBin == -1 || cBin >= nbins_cent) continue;
    
    Float_t Sumcand = chSum_4 + phSum_4 + neSum_4 + muSum_4;

 if(pfpt_4>=ptCutMC){
   if(cBin==0 && pfpt_4<ptCutCentralMC) { continue; } // cut data at 100 GeV for 0-5% central 
   hMC_noCut_eta->Fill(pfeta_4,weight);
   hMC_noCut_phi->Fill(pfphi_4,weight);
   hMC_noCut_eta_cent[cBin]->Fill(pfeta_4,weight);
   hMC_noCut_phi_cent[cBin]->Fill(pfphi_4,weight);
   hMC_noCut_phi_eta->Fill(pfeta_4,pfphi_4,weight);
    if(eMax_4/Sumcand < 0.05 ){
   hMC_CutA_eta->Fill(pfeta_4,weight);
   hMC_CutA_phi->Fill(pfphi_4,weight);
   hMC_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
   hMC_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);  
   hMC_CutA_phi_eta->Fill(pfeta_4,pfphi_4,weight);  
    } 
    
    if(jet55_4 == 1 && jet65_4==0 && jet80_4 == 0){
	  hMC_Jet55_noCut_eta->Fill(pfeta_4, weight);
	  hMC_Jet55_noCut_phi->Fill(pfphi_4, weight);
 	  hMC_Jet55_noCut_eta_cent[cBin]->Fill(pfeta_4, weight);
	  hMC_Jet55_noCut_phi_cent[cBin]->Fill(pfphi_4, weight);       
      hMC_unmatched_Jet55_noCut->Fill(pfrefpt_4, weight);
	  hMC_Jet55_noCut_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
      if(eMax_4/Sumcand < 0.05  ){
	hpbpb_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet55_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hMC_Jet55_CutA_eta->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi->Fill(pfphi_4, weight);
 	  hMC_Jet55_CutA_eta_cent[cBin]->Fill(pfeta_4, weight);
	  hMC_Jet55_CutA_phi_cent[cBin]->Fill(pfphi_4, weight); 
	  hMC_Jet55_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet55_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet55_data[cBin]->Fill(pfpt_4, weight);
	}
      
	hMC_unmatched_Jet55_CutA->Fill(pfrefpt_4, weight);}
      else hMC_unmatched_Jet55_CutA_rej->Fill(pfrefpt_4, weight);
      
    }

    
    if(jet65_4 == 1 && jet80_4 == 0){
      
      hMC_unmatched_Jet65_noCut->Fill(pfrefpt_4, weight);
      hMC_Jet65_noCut_eta->Fill(pfeta_4,weight);
      hMC_Jet65_noCut_phi->Fill(pfphi_4,weight);
      hMC_Jet65_noCut_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet65_noCut_phi_cent[cBin]->Fill(pfphi_4,weight);
      hMC_Jet65_noCut_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
      if(eMax_4/Sumcand < 0.05  ){
	hpbpb_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet65_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet65_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet65_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet65_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
	  hMC_Jet65_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet65_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet65_data[cBin]->Fill(pfpt_4, weight);
	}
      
	hMC_unmatched_Jet65_CutA->Fill(pfrefpt_4, weight);}
      else hMC_unmatched_Jet65_CutA_rej->Fill(pfrefpt_4);
      
    }

    
    if(jet80_4 == 1){
      
      hMC_unmatched_Jet80_noCut->Fill(pfrefpt_4, weight);
      hMC_Jet80_noCut_eta->Fill(pfeta_4,weight);
      hMC_Jet80_noCut_phi->Fill(pfphi_4,weight);
      hMC_Jet80_noCut_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet80_noCut_phi_cent[cBin]->Fill(pfphi_4,weight);
	  hMC_Jet80_noCut_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	  
      if(eMax_4/Sumcand < 0.05  ){
	hpbpb_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	hpbpb_Jet80_reco[cBin]->Fill(pfpt_4, weight);
	hpbpb_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
      hMC_Jet80_CutA_eta->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi->Fill(pfphi_4,weight);
      hMC_Jet80_CutA_eta_cent[cBin]->Fill(pfeta_4,weight);
      hMC_Jet80_CutA_phi_cent[cBin]->Fill(pfphi_4,weight);
	  hMC_Jet80_CutA_JetpTeta[cBin]->Fill(pfeta_4,pfpt_4,weight);
	if(nentry%2==0) {
	  hpbpb_mcclosure_matrix_HLT[cBin]->Fill(pfrefpt_4, pfpt_4, weight);
	  hpbpb_mcclosure_Jet80_gen[cBin]->Fill(pfrefpt_4, weight);
	}
	if(nentry%2==1) {
	  hpbpb_mcclosure_Jet80_data[cBin]->Fill(pfpt_4, weight);
	}
      
	hMC_unmatched_Jet80_CutA->Fill(pfrefpt_4, weight);}
      
      else hMC_unmatched_Jet80_CutA_rej->Fill(pfrefpt_4, weight);
      
    }
 }//end pT cut check MC loop
    
  }// mc unmatched  ntuple loop

  TFile fout(Form("PbPb_MinBias_BasicPlotsCutA_matched_slantedlinecalopfpt_addingunmatched_exclusionhighertriggers_eMaxSumcand_A_R0p%d.root",radius),"RECREATE");
 fout.cd();
  
  for(int i = 0;i<nbins_cent;++i){  //add histograms together
    hpbpb_TrgObjComb[i]->Add(hpbpb_TrgObj80[i]);
    hpbpb_TrgObjComb[i]->Add(hpbpb_TrgObj65[i]);
    hpbpb_TrgObjComb[i]->Add(hpbpb_TrgObj55[i]);

    hpbpb_mcclosure_JetComb_data[i]->Add(hpbpb_mcclosure_Jet80_data[i]);
    hpbpb_mcclosure_JetComb_data[i]->Add(hpbpb_mcclosure_Jet65_data[i]);
    hpbpb_mcclosure_JetComb_data[i]->Add(hpbpb_mcclosure_Jet55_data[i]);

    hpbpb_mcclosure_JetComb_gen[i]->Add(hpbpb_mcclosure_Jet80_gen[i]);
    hpbpb_mcclosure_JetComb_gen[i]->Add(hpbpb_mcclosure_Jet65_gen[i]);
    hpbpb_mcclosure_JetComb_gen[i]->Add(hpbpb_mcclosure_Jet55_gen[i]);

    hpbpb_JetComb_reco[i]->Add(hpbpb_Jet80_reco[i]);
    hpbpb_JetComb_reco[i]->Add(hpbpb_Jet65_reco[i]);
    hpbpb_JetComb_reco[i]->Add(hpbpb_Jet55_reco[i]);

    hpbpb_JetComb_gen[i]->Add(hpbpb_Jet80_gen[i]);
    hpbpb_JetComb_gen[i]->Add(hpbpb_Jet65_gen[i]);
    hpbpb_JetComb_gen[i]->Add(hpbpb_Jet55_gen[i]);
    
  }

  for(int i = 0; i<nbins_cent;++i){ //Write centrality histograms

    hpbpb_TrgObjComb[i]->Write();
    hpbpb_TrgObj80[i]->Write();
    hpbpb_TrgObj65[i]->Write();
    hpbpb_TrgObj55[i]->Write();

	//WRITE MATRIX
	cmatrix_HTL[i]->cd();
	hpbpb_matrix_HLT[i]->GetXaxis()->SetRangeUser(0,300);
	hpbpb_matrix_HLT[i]->GetYaxis()->SetRangeUser(0,300);
    hpbpb_matrix_HLT[i]->Write();
	cmatrix_HTL[i]->SetLogz();
    hpbpb_matrix_HLT[i]->Draw("colz");
	cmatrix_HTL[i]->SaveAs(Form("Plots/MinBias_hpbpb_MCmatrix_HLT_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	
	
	
	//WRITE pTeta
	hData_Jet55_noCut_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hData_Jet65_noCut_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hData_Jet80_noCut_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hData_Jet55_CutA_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hData_Jet65_CutA_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hData_Jet80_CutA_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	
	hData_Jet55_noCut_JetpTeta[i]->Write();
	hData_Jet55_CutA_JetpTeta[i]->Write();
	hData_Jet65_noCut_JetpTeta[i]->Write();
	hData_Jet65_CutA_JetpTeta[i]->Write();
	hData_Jet80_noCut_JetpTeta[i]->Write();
	hData_Jet80_CutA_JetpTeta[i]->Write();
	
	cPbPb_pteta_noCut[i]->cd();
	cPbPb_pteta_noCut[i]->Divide(3,1);
	cPbPb_pteta_noCut[i]->cd(1)->SetLogz();
	hData_Jet55_noCut_JetpTeta[i]->Draw("colz");
	cPbPb_pteta_noCut[i]->cd(2)->SetLogz();
	hData_Jet65_noCut_JetpTeta[i]->Draw("colz");
	cPbPb_pteta_noCut[i]->cd(3)->SetLogz();
	hData_Jet80_noCut_JetpTeta[i]->Draw("colz");
	cPbPb_pteta_noCut[i]->SaveAs(Form("Plots/MinBias_hData_pTeta_noCut_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	cPbPb_pteta_CutA[i]->cd();
	cPbPb_pteta_CutA[i]->Divide(3,1);
	cPbPb_pteta_CutA[i]->cd(1)->SetLogz();
	hData_Jet55_CutA_JetpTeta[i]->Draw("colz");
	cPbPb_pteta_CutA[i]->cd(2)->SetLogz();
	hData_Jet65_CutA_JetpTeta[i]->Draw("colz");
	cPbPb_pteta_CutA[i]->cd(3)->SetLogz();
	hData_Jet80_CutA_JetpTeta[i]->Draw("colz");
	cPbPb_pteta_CutA[i]->SaveAs(Form("Plots/MinBias_hData_pTeta_CutA_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	
	
	hMC_Jet55_noCut_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hMC_Jet65_noCut_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hMC_Jet80_noCut_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hMC_Jet55_CutA_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hMC_Jet65_CutA_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	hMC_Jet80_CutA_JetpTeta[i]->GetXaxis()->SetTitle("#eta");
	
	hMC_Jet55_noCut_JetpTeta[i]->Write();
	hMC_Jet55_CutA_JetpTeta[i]->Write();
	hMC_Jet65_noCut_JetpTeta[i]->Write();
	hMC_Jet65_CutA_JetpTeta[i]->Write();
	hMC_Jet80_noCut_JetpTeta[i]->Write();
	hMC_Jet80_CutA_JetpTeta[i]->Write();
	
	
	cMC_pteta_noCut[i]->cd();
	cMC_pteta_noCut[i]->Divide(3,1);
	cMC_pteta_noCut[i]->cd(1)->SetLogz();
	hMC_Jet55_noCut_JetpTeta[i]->Draw("colz");
	cMC_pteta_noCut[i]->cd(2)->SetLogz();
	hMC_Jet65_noCut_JetpTeta[i]->Draw("colz");
	cMC_pteta_noCut[i]->cd(3)->SetLogz();
	hMC_Jet80_noCut_JetpTeta[i]->Draw("colz");
	cMC_pteta_noCut[i]->SaveAs(Form("Plots/MinBias_hMC_pTeta_noCut_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	cMC_pteta_CutA[i]->cd();
	cMC_pteta_CutA[i]->Divide(3,1);
	cMC_pteta_CutA[i]->cd(1)->SetLogz();;
	hMC_Jet55_CutA_JetpTeta[i]->Draw("colz");
	cMC_pteta_CutA[i]->cd(2)->SetLogz();
	hMC_Jet65_CutA_JetpTeta[i]->Draw("colz");
	cMC_pteta_CutA[i]->cd(3)->SetLogz();
	hMC_Jet80_CutA_JetpTeta[i]->Draw("colz");
	cMC_pteta_CutA[i]->SaveAs(Form("Plots/MinBias_hMC_pTeta_CutA_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	
	
		
	//WRITE MC Folding Matrix
    hpbpb_matrix_HLT[i]->Write();
	hpbpb_mcclosure_matrix_HLT[i]->Write();
    hpbpb_mcclosure_JetComb_data[i]->Write();
    hpbpb_mcclosure_Jet80_data[i]->Write();
    hpbpb_mcclosure_Jet65_data[i]->Write();
    hpbpb_mcclosure_Jet55_data[i]->Write();
    hpbpb_mcclosure_JetComb_gen[i]->Write();    
    hpbpb_mcclosure_Jet80_gen[i]->Write();
    hpbpb_mcclosure_Jet65_gen[i]->Write();
    hpbpb_mcclosure_Jet55_gen[i]->Write();

    hpbpb_JetComb_reco[i]->Write();
    hpbpb_Jet80_reco[i]->Write();
    hpbpb_Jet65_reco[i]->Write();
    hpbpb_Jet55_reco[i]->Write();
    hpbpb_JetComb_gen[i]->Write();
    hpbpb_Jet80_gen[i]->Write();
    hpbpb_Jet65_gen[i]->Write();
    hpbpb_Jet55_gen[i]->Write();
    
 	  hMC_Jet55_noCut_eta_cent[i]->Write();
	  hMC_Jet55_noCut_phi_cent[i]->Write();
 	  hMC_Jet55_CutA_eta_cent[i]->Write();
	  hMC_Jet55_CutA_phi_cent[i]->Write();
      hMC_Jet65_noCut_eta_cent[i]->Write();
      hMC_Jet65_noCut_phi_cent[i]->Write();
      hMC_Jet65_CutA_eta_cent[i]->Write();
      hMC_Jet65_CutA_phi_cent[i]->Write();
      hMC_Jet80_noCut_eta_cent[i]->Write();
      hMC_Jet80_noCut_phi_cent[i]->Write();
      hMC_Jet80_CutA_eta_cent[i]->Write();
      hMC_Jet80_CutA_phi_cent[i]->Write();

 
  }
// new histograms save:
	  hMC_Jet55_noCut_eta->Write();
	  hMC_Jet55_noCut_phi->Write();
	  hMC_Jet55_CutA_eta->Write();
	  hMC_Jet55_CutA_phi->Write();
      hMC_Jet65_noCut_eta->Write();
      hMC_Jet65_noCut_phi->Write();
      hMC_Jet65_CutA_eta->Write();
      hMC_Jet65_CutA_phi->Write();
      hMC_Jet80_noCut_eta->Write();
      hMC_Jet80_noCut_phi->Write();
      hMC_Jet80_CutA_eta->Write();
      hMC_Jet80_CutA_phi->Write();
 
//WRITE Phi vs Eta dist.
 TCanvas * cMC_noCut_phi_eta = new TCanvas("cMC_noCut_phi_eta","",1000,800);
 cMC_noCut_phi_eta->cd();
 hMC_noCut_phi_eta->Write(); 
 hMC_noCut_phi_eta->Draw("colz"); 
 cMC_noCut_phi_eta->SaveAs(Form("Plots/MinBias_MC_noCut_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
 
 TCanvas * cMC_CutA_phi_eta = new TCanvas("cMC_noCut_phi_eta","",1000,800);
 cMC_CutA_phi_eta->cd();
 hMC_CutA_phi_eta->Write(); 
 hMC_CutA_phi_eta->Draw("colz"); 
 cMC_CutA_phi_eta->SaveAs(Form("Plots/MinBias_MC_CutA_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
 
 TCanvas * cPbPb_noCut_phi_eta = new TCanvas("cMC_noCut_phi_eta","",1000,800);
 cPbPb_noCut_phi_eta->cd();
 hData_noCut_phi_eta->Write(); 
 hData_noCut_phi_eta->Draw("colz"); 
 cPbPb_noCut_phi_eta->SaveAs(Form("Plots/MinBias_Data_noCut_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
 
 TCanvas * cPbPb_CutA_phi_eta = new TCanvas("cMC_noCut_phi_eta","",1000,800);
 cPbPb_CutA_phi_eta->cd();
 hData_CutA_phi_eta->Write();
 hData_CutA_phi_eta->Draw("colz");
 cPbPb_CutA_phi_eta->SaveAs(Form("Plots/MinBias_Data_CutA_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
 
 
 
  // add the unmatched histograms to the matched ones to get the final cut efficiency
  hData_Jet55_noCut->Add(hData_unmatched_Jet55_noCut);
  hData_Jet65_noCut->Add(hData_unmatched_Jet65_noCut);
  hData_Jet80_noCut->Add(hData_unmatched_Jet80_noCut);
  
  hData_Jet55_CutA->Add(hData_unmatched_Jet55_CutA);
  hData_Jet65_CutA->Add(hData_unmatched_Jet65_CutA);
  hData_Jet80_CutA->Add(hData_unmatched_Jet80_CutA);

  hData_Jet55_CutA_rej->Add(hData_unmatched_Jet55_CutA_rej);
  hData_Jet65_CutA_rej->Add(hData_unmatched_Jet65_CutA_rej);
  hData_Jet80_CutA_rej->Add(hData_unmatched_Jet80_CutA_rej);

  hMC_Jet55_noCut->Add(hMC_unmatched_Jet55_noCut);
  hMC_Jet65_noCut->Add(hMC_unmatched_Jet65_noCut);
  hMC_Jet80_noCut->Add(hMC_unmatched_Jet80_noCut);
  
  hMC_Jet55_CutA->Add(hMC_unmatched_Jet55_CutA);
  hMC_Jet65_CutA->Add(hMC_unmatched_Jet65_CutA);
  hMC_Jet80_CutA->Add(hMC_unmatched_Jet80_CutA);

  hMC_Jet55_CutA_rej->Add(hMC_unmatched_Jet55_CutA_rej);
  hMC_Jet65_CutA_rej->Add(hMC_unmatched_Jet65_CutA_rej);
  hMC_Jet80_CutA_rej->Add(hMC_unmatched_Jet80_CutA_rej);


  hData_Jet65_noCut->Write();
  hData_Jet65_CutA->Write();
  // hData_Jet65_CutB->Write();
  TH1F * hData_Jet65_CutA_eff = (TH1F*)hData_Jet65_CutA->Clone("hData_Jet65_CutA_eff");
  hData_Jet65_CutA_eff->Divide(hData_Jet65_noCut);
  hData_Jet65_CutA_eff->Write();
  // TH1F * hData_Jet65_CutB_eff = (TH1F*)hData_Jet65_CutB->Clone("hData_Jet65_CutB_eff");
  // hData_Jet65_CutB_eff->Divide(hData_Jet65_noCut);
  // hData_Jet65_CutB_eff->Write();
  hData_Jet65_CutA_rej->Write();
  // hData_Jet65_CutB_rej->Write();
  TH1F * hData_Jet65_CutA_rej_eff = (TH1F*)hData_Jet65_CutA_rej->Clone("hData_Jet65_CutA_rej_eff");
  hData_Jet65_CutA_rej_eff->Divide(hData_Jet65_noCut);
  hData_Jet65_CutA_rej_eff->Write();
  // hData_Jet65_CutB_rej_eff = (TH1F*)hData_Jet65_CutB_rej->Clone("hData_Jet65_CutB_rej_eff");
  // hData_Jet65_CutB_rej_eff->Divide(hData_Jet65_noCut);
  // hData_Jet65_CutB_rej_eff->Write();

  hData_Jet55_noCut->Write();
  hData_Jet55_CutA->Write();
  // hData_Jet55_CutB->Write();
  TH1F * hData_Jet55_CutA_eff = (TH1F*)hData_Jet55_CutA->Clone("hData_Jet55_CutA_eff");
  hData_Jet55_CutA_eff->Divide(hData_Jet55_noCut);
  hData_Jet55_CutA_eff->Write();
  // TH1F * hData_Jet55_CutB_eff = (TH1F*)hData_Jet55_CutB->Clone("hData_Jet55_CutB_eff");
  // hData_Jet55_CutB_eff->Divide(hData_Jet55_noCut);
  // hData_Jet55_CutB_eff->Write();
  hData_Jet55_CutA_rej->Write();
  // hData_Jet55_CutB_rej->Write();
  TH1F * hData_Jet55_CutA_rej_eff = (TH1F*)hData_Jet55_CutA_rej->Clone("hData_Jet55_CutA_rej_eff");
  hData_Jet55_CutA_rej_eff->Divide(hData_Jet55_noCut);
  hData_Jet55_CutA_rej_eff->Write();
  // hData_Jet55_CutB_rej_eff = (TH1F*)hData_Jet55_CutB_rej->Clone("hData_Jet55_CutB_rej_eff");
  // hData_Jet55_CutB_rej_eff->Divide(hData_Jet55_noCut);
  // hData_Jet55_CutB_rej_eff->Write();

  hData_Jet80_noCut->Write();
  hData_Jet80_CutA->Write();
  //hData_Jet80_CutB->Write();
  TH1F * hData_Jet80_CutA_eff = (TH1F*)hData_Jet80_CutA->Clone("hData_Jet80_CutA_eff");
  hData_Jet80_CutA_eff->Divide(hData_Jet80_noCut);
  hData_Jet80_CutA_eff->Write();
  // TH1F * hData_Jet80_CutB_eff = (TH1F*)hData_Jet80_CutB->Clone("hData_Jet80_CutB_eff");
  // hData_Jet80_CutB_eff->Divide(hData_Jet80_noCut);
  // hData_Jet80_CutB_eff->Write();
  hData_Jet80_CutA_rej->Write();
  // hData_Jet80_CutB_rej->Write();
  TH1F * hData_Jet80_CutA_rej_eff = (TH1F*)hData_Jet80_CutA_rej->Clone("hData_Jet80_CutA_rej_eff");
  hData_Jet80_CutA_rej_eff->Divide(hData_Jet80_noCut);
  hData_Jet80_CutA_rej_eff->Write();
  // hData_Jet80_CutB_rej_eff = (TH1F*)hData_Jet80_CutB_rej->Clone("hData_Jet80_CutB_rej_eff");
  // hData_Jet80_CutB_rej_eff->Divide(hData_Jet80_noCut);
  // hData_Jet80_CutB_rej_eff->Write();
  
  
  hMC_Jet80_noCut->Write();
  hMC_Jet80_CutA->Write();
  // hMC_Jet80_CutB->Write();
  TH1F * hMC_Jet80_CutA_eff = (TH1F*)hMC_Jet80_CutA->Clone("hMC_Jet80_CutA_eff");
  hMC_Jet80_CutA_eff->Divide(hMC_Jet80_noCut);
  hMC_Jet80_CutA_eff->Write();
  // TH1F * hMC_Jet80_CutB_eff = (TH1F*)hMC_Jet80_CutB->Clone("hMC_Jet80_CutB_eff");
  // hMC_Jet80_CutB_eff->Divide(hMC_Jet80_noCut);
  // hMC_Jet80_CutB_eff->Write();
  
  hMC_Jet65_noCut->Write();
  hMC_Jet65_CutA->Write();
  // hMC_Jet65_CutB->Write();
  TH1F * hMC_Jet65_CutA_eff = (TH1F*)hMC_Jet65_CutA->Clone("hMC_Jet65_CutA_eff");
  hMC_Jet65_CutA_eff->Divide(hMC_Jet65_noCut);
  hMC_Jet65_CutA_eff->Write();
  // TH1F * hMC_Jet65_CutB_eff = (TH1F*)hMC_Jet65_CutB->Clone("hMC_Jet65_CutB_eff");
  // hMC_Jet65_CutB_eff->Divide(hMC_Jet65_noCut);
  // hMC_Jet65_CutB_eff->Write();
  
  hMC_Jet55_noCut->Write();
  hMC_Jet55_CutA->Write();
  // hMC_Jet55_CutB->Write();
  TH1F * hMC_Jet55_CutA_eff = (TH1F*)hMC_Jet55_CutA->Clone("hMC_Jet55_CutA_eff");
  hMC_Jet55_CutA_eff->Divide(hMC_Jet55_noCut);
  hMC_Jet55_CutA_eff->Write();
  // TH1F * hMC_Jet55_CutB_eff = (TH1F*)hMC_Jet55_CutB->Clone("hMC_Jet55_CutB_eff");
  // hMC_Jet55_CutB_eff->Divide(hMC_Jet55_noCut);
  // hMC_Jet55_CutB_eff->Write();

  // save the unmatched histograms as well:
  hData_unmatched_Jet80_noCut->Write();
  hData_unmatched_Jet80_CutA->Write();
  hData_unmatched_Jet80_CutA_rej->Write();
  hData_unmatched_Jet65_noCut->Write();
  hData_unmatched_Jet65_CutA->Write();
  hData_unmatched_Jet65_CutA_rej->Write();
  hData_unmatched_Jet55_noCut->Write();
  hData_unmatched_Jet55_CutA->Write();
  hData_unmatched_Jet55_CutA_rej->Write();

  hMC_unmatched_Jet80_noCut->Write();
  hMC_unmatched_Jet80_CutA->Write();
  hMC_unmatched_Jet80_CutA_rej->Write();
  hMC_unmatched_Jet65_noCut->Write();
  hMC_unmatched_Jet65_CutA->Write();
  hMC_unmatched_Jet65_CutA_rej->Write();
  hMC_unmatched_Jet55_noCut->Write();
  hMC_unmatched_Jet55_CutA->Write();
  hMC_unmatched_Jet55_CutA_rej->Write();
  
  TCanvas * cJet80_CutEfficiency_Jet80 = new TCanvas("cJet80_CutEfficiency_Jet80","",1000,800);
  cJet80_CutEfficiency_Jet80->Divide(2,1);
  cJet80_CutEfficiency_Jet80->cd(1);

  hData_Jet80_CutA_eff->SetMarkerColor(kRed);
  hData_Jet80_CutA_eff->SetMarkerStyle(24);
//  hData_Jet80_CutA_eff->SetAxisRange(30,300,"X");
//  hData_Jet80_CutA_eff->SetAxisRange(0.85,1.1,"Y");
  hData_Jet80_CutA_eff->Rebin(5);
  hData_Jet80_CutA_eff->Scale(1./5);
  hData_Jet80_CutA_eff->SetXTitle(Form("akPu%dPF p_{T}",radius));
  hData_Jet80_CutA_eff->SetTitle("Data");
  hData_Jet80_CutA_eff->SetYTitle("Jet80_Cut efficiency");
  hData_Jet80_CutA_eff->Draw();
  // hData_Jet80_CutB_eff->Rebin(20);
  // hData_Jet80_CutB_eff->Scale(1./20);
  // hData_Jet80_CutB_eff->SetMarkerColor(kBlack);
  // hData_Jet80_CutB_eff->SetMarkerStyle(33);
  // hData_Jet80_CutB_eff->SetAxisRange(20,600,"X");
  // hData_Jet80_CutB_eff->Draw("same");

  cJet80_CutEfficiency_Jet80->cd(2);
  hMC_Jet80_CutA_eff->SetMarkerColor(kRed);
  hMC_Jet80_CutA_eff->SetMarkerStyle(24);
//  hMC_Jet80_CutA_eff->SetAxisRange(30,300,"X");
//  hMC_Jet80_CutA_eff->SetAxisRange(0.85,1.1,"Y");
  hMC_Jet80_CutA_eff->Rebin(5);
  hMC_Jet80_CutA_eff->Scale(1./5);
  hMC_Jet80_CutA_eff->SetXTitle(Form("akPu%dPF ref p_{T}",radius));
  hMC_Jet80_CutA_eff->SetTitle("MC");
  hMC_Jet80_CutA_eff->SetYTitle("Jet80_Cut efficiency");
  hMC_Jet80_CutA_eff->Draw();
  // hMC_Jet80_CutB_eff->Rebin(20);
  // hMC_Jet80_CutB_eff->Scale(1./20);
  // hMC_Jet80_CutB_eff->SetMarkerColor(kBlack);
  // hMC_Jet80_CutB_eff->SetMarkerStyle(33);
  // hMC_Jet80_CutB_eff->SetAxisRange(20,600,"X");
  // hMC_Jet80_CutB_eff->Draw("same");

  cJet80_CutEfficiency_Jet80->SaveAs(Form("Plots/MinBias_PbPb_YetkinCuts_Jet80_eMaxSumcand_A_chMaxJtpt_calopfpt_Eff_R0p%d_zoomed.pdf",radius));
 cJet80_CutEfficiency_Jet80->Close();
 
  // TCanvas * cCutRejection_Jet80 = new TCanvas("cCutRejection_Jet80","",1000,800);

  // hData_Jet80_CutA_rej->Rebin(5);
  // hData_Jet80_CutA_rej->Scale(1./5);
  // hData_Jet80_CutA_rej->SetMarkerColor(kRed);
  // hData_Jet80_CutA_rej->SetMarkerStyle(24);
  // hData_Jet80_CutA_rej->SetAxisRange(20,600,"X");
  // hData_Jet80_CutA_rej->SetAxisRange(0,1.2,"Y");
  // hData_Jet80_CutA_rej->SetXTitle("matched akPu3PF p_{T}");
  // hData_Jet80_CutA_rej->SetTitle("Data");
  // hData_Jet80_CutA_rej->SetYTitle("Jet80_Cut Rejection");
  // hData_Jet80_CutA_rej->Draw();
  // // hData_Jet80_CutB_rej->Rebin(20);
  // // hData_Jet80_CutB_rej->Scale(1./20);
  // // hData_Jet80_CutB_rej->SetMarkerColor(kBlack);
  // // hData_Jet80_CutB_rej->SetMarkerStyle(33);
  // // hData_Jet80_CutB_rej->SetAxisRange(20,600,"X");
  // // hData_Jet80_CutB_rej->Draw("same");

  // cCutRejection_Jet80->SaveAs("Plots/MinBias_PbPb_YetkinCuts_Jet80_eMaxSumcand_A_chMaxJtpt_calopfpt_rejection.pdf","RECREATE");

  TCanvas * cJet55_CutEfficiency_Jet55 = new TCanvas("cJet55_CutEfficiency_Jet55","",1000,800);
  cJet55_CutEfficiency_Jet55->Divide(2,1);
  cJet55_CutEfficiency_Jet55->cd(1);


  hData_Jet55_CutA_eff->SetMarkerColor(kRed);
  hData_Jet55_CutA_eff->SetMarkerStyle(24);
//  hData_Jet55_CutA_eff->SetAxisRange(30,300,"X");
//  hData_Jet55_CutA_eff->SetAxisRange(0.85,1.1,"Y");
  hData_Jet55_CutA_eff->Rebin(5);
  hData_Jet55_CutA_eff->Scale(1./5);
  hData_Jet55_CutA_eff->SetXTitle(Form("akPu%dPF p_{T}",radius));
  hData_Jet55_CutA_eff->SetTitle("Data");
  hData_Jet55_CutA_eff->SetYTitle("Jet55_Cut efficiency");
  hData_Jet55_CutA_eff->Draw();
  // hData_Jet55_CutB_eff->Rebin(20);
  // hData_Jet55_CutB_eff->Scale(1./20);
  // hData_Jet55_CutB_eff->SetMarkerColor(kBlack);
  // hData_Jet55_CutB_eff->SetMarkerStyle(33);
  // hData_Jet55_CutB_eff->SetAxisRange(20,600,"X");
  // hData_Jet55_CutB_eff->SetAxisRange(0,1.2,"Y");
  // hData_Jet55_CutB_eff->Draw("same");

  cJet55_CutEfficiency_Jet55->cd(2);
  hMC_Jet55_CutA_eff->SetMarkerColor(kRed);
  hMC_Jet55_CutA_eff->SetMarkerStyle(24);
//  hMC_Jet55_CutA_eff->SetAxisRange(30,300,"X");
//  hMC_Jet55_CutA_eff->SetAxisRange(0.85,1.1,"Y");
  hMC_Jet55_CutA_eff->Rebin(5);
  hMC_Jet55_CutA_eff->Scale(1./5);
  hMC_Jet55_CutA_eff->SetXTitle(Form("akPu%dPF ref p_{T}",radius));
  hMC_Jet55_CutA_eff->SetTitle("MC");
  hMC_Jet55_CutA_eff->SetYTitle("Jet55_Cut efficiency");
  hMC_Jet55_CutA_eff->Draw();
  // hMC_Jet55_CutB_eff->Rebin(20);
  // hMC_Jet55_CutB_eff->Scale(1./20);
  // hMC_Jet55_CutB_eff->SetMarkerColor(kBlack);
  // hMC_Jet55_CutB_eff->SetMarkerStyle(33);
  // hMC_Jet55_CutB_eff->SetAxisRange(20,600,"X");
  // hMC_Jet55_CutB_eff->Draw("same");

  cJet55_CutEfficiency_Jet55->SaveAs(Form("Plots/MinBias_PbPb_YetkinCuts_Jet55_noJet65noJet80_eMaxSumcand_A_chMaxJtpt_calopfpt_Eff_including_unmatched_R0p%d_zoomed.pdf",radius));
  cJet55_CutEfficiency_Jet55->Close();
  // TCanvas * cCutRejection_Jet55 = new TCanvas("cCutRejection_Jet55","",1000,800);

  // hData_Jet55_CutA_rej->Rebin(5);
  // hData_Jet55_CutA_rej->Scale(1./5);
  // hData_Jet55_CutA_rej->SetMarkerColor(kRed);
  // hData_Jet55_CutA_rej->SetMarkerStyle(24);
  // hData_Jet55_CutA_rej->SetAxisRange(20,600,"X");
  // hData_Jet55_CutA_rej->SetAxisRange(0,1.2,"Y");
  // hData_Jet55_CutA_rej->SetXTitle("matched akPu3PF p_{T}");
  // hData_Jet55_CutA_rej->SetTitle("Data");
  // hData_Jet55_CutA_rej->SetYTitle("Jet55_Cut Rejection");
  // hData_Jet55_CutA_rej->Draw();
  // // hData_Jet55_CutB_rej->Rebin(20);
  // // hData_Jet55_CutB_rej->Scale(1./20);
  // // hData_Jet55_CutB_rej->SetMarkerColor(kBlack);
  // // hData_Jet55_CutB_rej->SetMarkerStyle(33);
  // // hData_Jet55_CutB_rej->SetAxisRange(20,600,"X");
  // // hData_Jet55_CutB_rej->Draw("same");

  // cCutRejection_Jet55->SaveAs("Plots/MinBias_PbPb_YetkinCuts_Jet55_noJet65noJet80_eMaxSumcand_A_chMaxJtpt_calopfpt_rejection_including_unmatched.pdf","RECREATE");

  TCanvas * cJet65_CutEfficiency_Jet65 = new TCanvas("cJet65_CutEfficiency_Jet65","",1000,800);
  cJet65_CutEfficiency_Jet65->Divide(2,1);
  cJet65_CutEfficiency_Jet65->cd(1);

  hData_Jet65_CutA_eff->SetMarkerColor(kRed);
  hData_Jet65_CutA_eff->SetMarkerStyle(24);
//  hData_Jet65_CutA_eff->SetAxisRange(30,300,"X");
//  hData_Jet65_CutA_eff->SetAxisRange(0.85,1.1,"Y");
  hData_Jet65_CutA_eff->Rebin(5);
  hData_Jet65_CutA_eff->Scale(1./5);
  hData_Jet65_CutA_eff->SetXTitle(Form("matched akPu%dPF p_{T}",radius));
  hData_Jet65_CutA_eff->SetTitle("Data");
  hData_Jet65_CutA_eff->SetYTitle("Jet65_Cut efficiency");
  hData_Jet65_CutA_eff->Draw();
  // hData_Jet65_CutB_eff->Rebin(20);
  // hData_Jet65_CutB_eff->Scale(1./20);
  // hData_Jet65_CutB_eff->SetMarkerColor(kBlack);
  // hData_Jet65_CutB_eff->SetMarkerStyle(33);
  // hData_Jet65_CutB_eff->SetAxisRange(20,600,"X");
  // hData_Jet65_CutB_eff->Draw("same");

  cJet65_CutEfficiency_Jet65->cd(2);
  hMC_Jet65_CutA_eff->SetMarkerColor(kRed);
  hMC_Jet65_CutA_eff->SetMarkerStyle(24);
//  hMC_Jet65_CutA_eff->SetAxisRange(30,300,"X");
//  hMC_Jet65_CutA_eff->SetAxisRange(0.85,1.1,"Y");
  hMC_Jet65_CutA_eff->Rebin(5);
  hMC_Jet65_CutA_eff->Scale(1./5);
  hMC_Jet65_CutA_eff->SetXTitle(Form("matched akPu%dPF ref p_{T}",radius));
  hMC_Jet65_CutA_eff->SetTitle("MC");
  hMC_Jet65_CutA_eff->SetYTitle("Jet65_Cut efficiency");
  hMC_Jet65_CutA_eff->Draw();
  // hMC_Jet65_CutB_eff->Rebin(20);
  // hMC_Jet65_CutB_eff->Scale(1./20);
  // hMC_Jet65_CutB_eff->SetMarkerColor(kBlack);
  // hMC_Jet65_CutB_eff->SetMarkerStyle(33);
  // hMC_Jet65_CutB_eff->SetAxisRange(20,600,"X");
  // hMC_Jet65_CutB_eff->Draw("same");

  cJet65_CutEfficiency_Jet65->SaveAs(Form("Plots/MinBias_PbPb_YetkinCuts_Jet65_noJet80_eMaxSumcand_A_chMaxJtpt_calopfpt_Eff_including_unmatched_R0p%d_zoomed.pdf",radius));
cJet65_CutEfficiency_Jet65->Close();
  // TCanvas * cCutRejection_Jet65 = new TCanvas("cCutRejection_Jet65","",1000,800);

  // hData_Jet65_CutA_rej->Rebin(5);
  // hData_Jet65_CutA_rej->Scale(1./5);
  // hData_Jet65_CutA_rej->SetMarkerColor(kRed);
  // hData_Jet65_CutA_rej->SetMarkerStyle(24);
  // hData_Jet65_CutA_rej->SetAxisRange(20,600,"X");
  // hData_Jet65_CutA_rej->SetAxisRange(0, 1.2,"Y");
  // hData_Jet65_CutA_rej->SetXTitle("matched akPu3PF p_{T}");
  // hData_Jet65_CutA_rej->SetTitle("Data");
  // hData_Jet65_CutA_rej->SetYTitle("Jet65_Cut Rejection");
  // hData_Jet65_CutA_rej->Draw();
  // // hData_Jet65_CutB_rej->Rebin(20);
  // // hData_Jet65_CutB_rej->Scale(1./20);
  // // hData_Jet65_CutB_rej->SetMarkerColor(kBlack);
  // // hData_Jet65_CutB_rej->SetMarkerStyle(33);
  // // hData_Jet65_CutB_rej->SetAxisRange(20,600,"X");
  // // hData_Jet65_CutB_rej->Draw("same");

  // cCutRejection_Jet65->SaveAs("Plots/MinBias_PbPb_YetkinCuts_Jet65_noJet80_eMaxSumcand_A_chMaxJtpt_calopfpt_rejection_including_unmatched.pdf","RECREATE");


  // plot the trigger combination from this, and the total cut efficiency:
  
  TH1F * hData_Jet80 = (TH1F*)hData_Jet80_CutA->Clone("hData_Jet80");
  TH1F * hData_Jet65 = (TH1F*)hData_Jet65_CutA->Clone("hData_Jet65");
  TH1F * hData_Jet55 = (TH1F*)hData_Jet55_CutA->Clone("hData_Jet55");

  TH1F * hData_Combined = (TH1F*)hData_Jet80->Clone("hData_Combined");
  hData_Combined->Add(hData_Jet65);
  hData_Combined->Add(hData_Jet55);

  hData_Combined->Print("base");
  
  TH1F * hData_noCut_Jet80 = (TH1F*)hData_Jet80_noCut->Clone("hData_noCut_Jet80");
  TH1F * hData_noCut_Jet65 = (TH1F*)hData_Jet65_noCut->Clone("hData_noCut_Jet65");
  TH1F * hData_noCut_Jet55 = (TH1F*)hData_Jet55_noCut->Clone("hData_noCut_Jet55");

  TH1F * hData_noCut_Combined = (TH1F*)hData_noCut_Jet80->Clone("hData_noCut_Combined");
  hData_noCut_Combined->Add(hData_noCut_Jet65);
  hData_noCut_Combined->Add(hData_noCut_Jet55);
  
  hData_noCut_Combined->Print("base");
  
  TH1F * hMC_Jet80 = (TH1F*)hMC_Jet80_CutA->Clone("hMC_Jet80");
  TH1F * hMC_Jet65 = (TH1F*)hMC_Jet65_CutA->Clone("hMC_Jet65");
  TH1F * hMC_Jet55 = (TH1F*)hMC_Jet55_CutA->Clone("hMC_Jet55");

  TH1F * hMC_Combined = (TH1F*)hMC_Jet80->Clone("hMC_Combined");
  hMC_Combined->Add(hMC_Jet65);
  hMC_Combined->Add(hMC_Jet55);

  hMC_Combined->Print("base");
  
  TH1F * hMC_noCut_Jet80 = (TH1F*)hMC_Jet80_noCut->Clone("hMC_noCut_Jet80");
  TH1F * hMC_noCut_Jet65 = (TH1F*)hMC_Jet65_noCut->Clone("hMC_noCut_Jet65");
  TH1F * hMC_noCut_Jet55 = (TH1F*)hMC_Jet55_noCut->Clone("hMC_noCut_Jet55");

  TH1F * hMC_noCut_Combined = (TH1F*)hMC_noCut_Jet80->Clone("hMC_noCut_Combined");
  hMC_noCut_Combined->Add(hMC_noCut_Jet65);
  hMC_noCut_Combined->Add(hMC_noCut_Jet55);

  hMC_noCut_Combined->Print("base");
  
  TH1F * hData_Combined_Efficiency = (TH1F*)hData_Combined->Clone("hData_Combined_Efficiency");
  hData_Combined_Efficiency->Divide(hData_noCut_Combined);
  hData_Combined_Efficiency->Print("base");
  
  TH1F * hMC_Combined_Efficiency = (TH1F*)hMC_Combined->Clone("hMC_Combined_Efficiency");
  hMC_Combined_Efficiency->Divide(hMC_noCut_Combined);
  hMC_Combined_Efficiency->Print("base");
  
  TCanvas * cCombinedEff = new TCanvas("cCombinedEff","",800,600);
  hData_Combined_Efficiency->SetXTitle("Jet p_{T}");
  hData_Combined_Efficiency->SetYTitle("Combined Jet ID cut efficiency");
  hData_Combined_Efficiency->SetMarkerStyle(20);
  hData_Combined_Efficiency->SetMarkerColor(kBlack);
  hData_Combined_Efficiency->SetAxisRange(30,350,"X");
  hData_Combined_Efficiency->Rebin(20);
  hData_Combined_Efficiency->Scale(1/20);
  hData_Combined_Efficiency->Draw();
  
  hMC_Combined_Efficiency->SetMarkerStyle(24);
  hMC_Combined_Efficiency->SetMarkerColor(kRed);
  hMC_Combined_Efficiency->SetAxisRange(30,350,"X");
  hMC_Combined_Efficiency->Rebin(20);
  hMC_Combined_Efficiency->Scale(1/20);
  hMC_Combined_Efficiency->Draw("same");

  cCombinedEff->SaveAs(Form("Plots/MinBias_Combined_trigger_efficiency_YetkinCut_R0p%d.pdf",radius));
  cCombinedEff->Close();
  
  TCanvas * cTriggerCombination = new TCanvas("cTriggerCombination","",800,600);
  cTriggerCombination->SetLogy();

  hData_Combined->SetMarkerColor(kBlack);
  hData_Combined->SetMarkerStyle(25);
  hData_Combined->SetAxisRange(30,350,"X");
  hData_Combined->SetXTitle("");
  hData_Combined->Draw();

  hData_Jet80->SetMarkerColor(kRed);
  hData_Jet80->SetMarkerStyle(20);
  hData_Jet80->Draw("same");
  
  hData_Jet65->SetMarkerColor(kBlue);
  hData_Jet65->SetMarkerStyle(20);
  hData_Jet65->Draw("same");

  hData_Jet55->SetMarkerColor(kGreen);
  hData_Jet55->SetMarkerStyle(20);
  hData_Jet55->Draw("same");

  //drawText()

  cTriggerCombination->SaveAs(Form("Plots/MinBias_TriggerCombination_YetkinCuts_R0%d.pdf",radius));
  cTriggerCombination->Close();

  TCanvas * cenoCut_eta_cent[nbins_cent];
  TCanvas * cenoCut_phi_cent[nbins_cent];

  for(int i = 0 ; i < nbins_cent ; ++i){

    cenoCut_eta_cent[i] = new TCanvas(Form("cenoCut_eta_cent%d",i),"",800,600);
    cenoCut_eta_cent[i]->cd();
    cenoCut_eta_cent[i]->SetLogy();
    hMC_noCut_eta_cent[i]->SetMarkerColor(kBlue+1);
    hMC_noCut_eta_cent[i]->SetMarkerStyle(25);
    hMC_noCut_eta_cent[i]->Print("base");
    hMC_noCut_eta_cent[i]->SetTitle(" ");
    hMC_noCut_eta_cent[i]->SetXTitle("ak3PuPF jet eta no cuts");
    hMC_noCut_eta_cent[i]->DrawNormalized();

    hData_noCut_eta_cent[i]->SetMarkerStyle(24);
    hData_noCut_eta_cent[i]->SetMarkerColor(kBlack);
    hData_noCut_eta_cent[i]->DrawNormalized("same");
    drawText(Form("%2.0f-%2.0f%%",2.5*boundaries_cent[i],2.5*boundaries_cent[i+1]),0.8,0.8,16);

    TLegend * leg = myLegend(0.15,0.15,0.3,0.3);
    leg->AddEntry(hMC_noCut_eta_cent[0],"MC","pl");
    leg->AddEntry(hData_noCut_eta_cent[0],"Data","pl");
    leg->SetTextSize(0.04);
    leg->Draw();
    
    cenoCut_eta_cent[i]->SaveAs(Form("Plots/MinBias_NoCuts_eta_R%d_cent%d_MCweight.pdf",radius,i),"RECREATE");
    cenoCut_eta_cent[i]->Close();
  }
 
  
  for(int i = 0 ; i < nbins_cent ; ++i){

    cenoCut_phi_cent[i] = new TCanvas(Form("cenoCut_phi_cent%d",i),"",800,600);
    cenoCut_phi_cent[i]->cd();
    cenoCut_phi_cent[i]->SetLogy();
    hMC_noCut_phi_cent[i]->SetMarkerColor(kRed);
    hMC_noCut_phi_cent[i]->SetMarkerStyle(28);
    hMC_noCut_phi_cent[i]->Print("base");
    hMC_noCut_phi_cent[i]->SetTitle(" ");
    hMC_noCut_phi_cent[i]->SetXTitle("ak3PuPF jet phi no cuts");
    hMC_noCut_phi_cent[i]->DrawNormalized();

    hData_noCut_phi_cent[i]->SetMarkerStyle(24);
    hData_noCut_phi_cent[i]->SetMarkerColor(kBlack);
    hData_noCut_phi_cent[i]->DrawNormalized("same");
    drawText(Form("%2.0f-%2.0f%%",2.5*boundaries_cent[i],2.5*boundaries_cent[i+1]),0.8,0.8,16);

    TLegend * leg = myLegend(0.15,0.15,0.3,0.3);
    leg->AddEntry(hMC_noCut_phi_cent[0],"MC","pl");
    leg->AddEntry(hData_noCut_phi_cent[0],"Data","pl");
    leg->SetTextSize(0.04);
    leg->Draw();
    
    cenoCut_phi_cent[i]->SaveAs(Form("Plots/MinBias_NoCuts_R%d_phi_cent%d_MCweight.pdf",radius,i),"RECREATE");
    cenoCut_phi_cent[i]->Close();
  }
  TCanvas * ceCutA_eta_cent[nbins_cent];
  TCanvas * ceCutA_phi_cent[nbins_cent];

  for(int i = 0 ; i < nbins_cent ; ++i){

    ceCutA_eta_cent[i] = new TCanvas(Form("ceCutA_eta_cent%d",i),"",800,600);
    ceCutA_eta_cent[i]->cd();
    ceCutA_eta_cent[i]->SetLogy();
    hMC_CutA_eta_cent[i]->SetMarkerColor(kBlue+1);
    hMC_CutA_eta_cent[i]->SetMarkerStyle(25);
    hMC_CutA_eta_cent[i]->Print("base");
    hMC_CutA_eta_cent[i]->SetTitle(" ");
    hMC_CutA_eta_cent[i]->SetXTitle("ak3PuPF jet eta CutA");
    hMC_CutA_eta_cent[i]->DrawNormalized();

    hData_CutA_eta_cent[i]->SetMarkerStyle(24);
    hData_CutA_eta_cent[i]->SetMarkerColor(kBlack);
    hData_CutA_eta_cent[i]->DrawNormalized("same");
    drawText(Form("%2.0f-%2.0f%%",2.5*boundaries_cent[i],2.5*boundaries_cent[i+1]),0.8,0.8,16);

    TLegend * leg = myLegend(0.15,0.15,0.3,0.3);
    leg->AddEntry(hMC_CutA_eta_cent[0],"MC","pl");
    leg->AddEntry(hData_CutA_eta_cent[0],"Data","pl");
    leg->SetTextSize(0.04);
    leg->Draw();
    
    ceCutA_eta_cent[i]->SaveAs(Form("Plots/MinBias_CutA_R%d_eta_cent%d_MCweight.pdf",radius,i),"RECREATE");
    ceCutA_eta_cent[i]->Close();
  }
  for(int i = 0 ; i < nbins_cent ; ++i){

    ceCutA_phi_cent[i] = new TCanvas(Form("ceCutA_phi_cent%d",i),"",800,600);
    ceCutA_phi_cent[i]->cd();
    ceCutA_phi_cent[i]->SetLogy();
    hMC_CutA_phi_cent[i]->SetMarkerColor(kRed);
    hMC_CutA_phi_cent[i]->SetMarkerStyle(28);
    hMC_CutA_phi_cent[i]->Print("base");
    hMC_CutA_phi_cent[i]->SetTitle(" ");
    hMC_CutA_phi_cent[i]->SetXTitle("ak3PuPF jet phi CutA");
    hMC_CutA_phi_cent[i]->DrawNormalized();

    hData_CutA_phi_cent[i]->SetMarkerStyle(24);
    hData_CutA_phi_cent[i]->SetMarkerColor(kBlack);
    hData_CutA_phi_cent[i]->DrawNormalized("same");
    drawText(Form("%2.0f-%2.0f%%",2.5*boundaries_cent[i],2.5*boundaries_cent[i+1]),0.8,0.8,16);

    TLegend * leg = myLegend(0.15,0.15,0.3,0.3);
    leg->AddEntry(hMC_CutA_phi_cent[0],"MC","pl");
    leg->AddEntry(hData_CutA_phi_cent[0],"Data","pl");
    leg->SetTextSize(0.04);
    leg->Draw();
    
    ceCutA_phi_cent[i]->SaveAs(Form("Plots/MinBias_CutA_R%d_phi_cent%d_MCweight.pdf",radius,i),"RECREATE");
    ceCutA_phi_cent[i]->Close();
  }
  
  cout<<"line 1892"<<endl;
  cPbPb_pteta80_noCut->Divide(3,2);
	  cPbPb_pteta80_noCut->SetLogz();
	  cout<<"start stdarray loop - debug this!"<<endl;
	  
//   for(int i = 0 ; i < nbins_cent ; ++i){
// 	cPbPb_pteta80_noCut->cd(stdarray[i])->SetLogz();
// 	hData_Jet80_noCut_JetpTeta[i]->Draw("colz");
// 	  }
// 	  cPbPb_pteta80_noCut->SaveAs(Form("Plots/MinBias_hData_pTeta80_noCut_R%d_n20_eta_p20_allcent.pdf",radius),"RECREATE");
	  
	  cout<<"finish macro"<<endl;
}
