...
#include "/net/hisrv0001/home/salur/hibat7/CMSSW_5_3_20/src/Headers/RooUnfold-1.1.1/src/RooUnfoldResponse.h"
#include "/net/hisrv0001/home/salur/hibat7/CMSSW_5_3_20/src/Headers/RooUnfold-1.1.1/src/RooUnfoldBayes.h"
#include "/net/hisrv0001/home/salur/hibat7/CMSSW_5_3_20/src/Headers/RooUnfold-1.1.1/src/RooUnfoldSvd.h"
#include "/net/hisrv0001/home/salur/hibat7/CMSSW_5_3_20/src/Headers/RooUnfold-1.1.1/src/RooUnfoldBinByBin.h"
#include "/net/hisrv0001/home/salur/hibat7/CMSSW_5_3_20/src/Headers/prior.h"
#include "/net/hisrv0001/home/salur/hibat7/CMSSW_5_3_20/src/Headers/bayesianUnfold.h"
...
static const int nbins_pt_fine = 400;
...
void RAA_analyze(int radius = 2, char* algo = (char*) "Pu", char *jet_type = (char*) "PF", int unfoldingCut = 40, char* etaWidth = (char*) "10_eta_18", double deltaEta = 1.6){
  //gSystem->Load("/net/hisrv0001/home/salur/hibat7/CMSSW_5_3_20/src/Headers/RooUnfold-1.1.1/libRooUnfold.so");
  TStopwatch timer; 
  timer.Start();
...
//= TFile::Open(Form("/afs/cern.ch/work/r/rkunnawa/WORK/RAA/CMSSW_5_3_18/src/Output/Pawan_ntuple_PbPb_data_MC_subid0_spectra_JetID_CutA_finebins_%s_R0p%d.root",etaWidth, radius)); 
  ...
  //    TFile *  fMinBias = TFile::Open(Form("Pawan_ntuple_PbPb_MinBiasDatawithoutHLT_spectra_JetID_CutA_finebins_CentralityWeighted_%s_R0p%d.root",etaWidth,radius)); //MinBias File 

  //TFile *  fMinBias = TFile::Open(Form("Pawan_ntuple_PbPb_MinBiasData_spectra_JetID_CutA_finebins_CentralityWeightedNoHLT_%s_R0p%d.root",etaWidth,radius)); //MinBias File 
  ...
  // we need the get the mc histograms from matched + unmatched
  TH1F * mPbPb_mcclosure_data_m[nbins_cent+1], * mPbPb_mcclosure_gen_m[nbins_cent+1];
  TH2F * mPbPb_mcclosure_matrix_m[nbins_cent+1]; 
  TH1F * mPbPb_mcclosure_data_um[nbins_cent+1], * mPbPb_mcclosure_gen_um[nbins_cent+1];
  TH2F * mPbPb_mcclosure_matrix_um[nbins_cent+1];
  ...
  TH1F * htest = new TH1F("htest","",400,0,400);
  ...
  for(int i = 0;i<nbins_cent;++i){
    ...
    //mPbPb_Gen[i] = (TH1F*)fPbPb_in->Get(Form("hpbpb_gen_R%d_%s_cent%d",radius,etaWidth,i));
    mPbPb_Gen[i]->Print("base");
    mPbPb_Reco[i] = (TH1F*)fPbPb_in->Get(Form("hpbpb_JetComb_reco_R%d_%s_cent%d",radius,etaWidth,i));
    //mPbPb_Reco[i] = (TH1F*)fPbPb_in->Get(Form("hpbpb_reco_R%d_%s_cent%d",radius,etaWidth,i));
    ...
    */
    ...
    // mPbPb_mcclosure_data_um[i] = (TH1F*)fMCClosure->Get(Form("akPu%dJetAnalyzer/hrec_um_c_f_pbpb_akPu%d_1_%d",radius, radius, i));
    // mPbPb_mcclosure_data_um[i]->Print("base");
    // mPbPb_mcclosure_gen_um[i] = (TH1F*)fMCClosure->Get(Form("akPu%dJetAnalyzer/hgen_um_f_pbpb_akPu%d_1_%d",radius, radius, i));
    // mPbPb_mcclosure_gen_um[i]->Print("base");

    // // now add the histograms.
    // mPbPb_mcclosure_data[i] = (TH1F*) mPbPb_mcclosure_data_m[i]->Clone(Form("hrec_c_f_pbpb_akPu%d_1_%d",radius,i));
    // mPbPb_mcclosure_data[i]->Add(mPbPb_mcclosure_data_um[i]);
    // mPbPb_mcclosure_gen[i] = (TH1F*) mPbPb_mcclosure_gen_m[i]->Clone(Form("hgen_f_pbpb_akPu%d_1_%d",radius,i));
    // mPbPb_mcclosure_gen[i]->Add(mPbPb_mcclosure_gen_um[i]);
    ...
    if(etaWidth == "20_eta_20"){
      ...
      if(i == 0 && radius==3) unfoldingCutBin = htest->FindBin(70); //seviltest 70 
      if(i == 1 && radius==3) unfoldingCutBin = htest->FindBin(60); //60
  ...
  mPP_mcclosure_Matrix = (TH2F*)fMCClosure->Get(Form("ak%dJetAnalyzer/hmatrix_f_pp_ak%d_1_7",radius,radius));
  ...
  TFile * ferrorin = TFile::Open(Form("/export/d00/scratch/rkunnawa/rootfiles/RAA/May6/Pawan_ntuple_PbPb_R%d_pp_R%d_noJetID_%s_unfoldingCut_%d_noFakeWeight_data_driven_correction_akPu%s_20150506.root",radius, radius, etaWidth, unfoldingCut, jet_type));
  ...
  for(int i = 0;i<nbins_cent;++i){
    ...
    RAA_JEC_bayesian[i]->Scale(1./145.156); // triggered value
    ...
    RAA_JEC_measured[i]->Scale(1./145.156);
    ...
    RAA_JEC_binbybin[i]->Scale(1./145.156);
    ...
    RAA_Smear_bayesian[i]->Scale(1./145.156); // triggered value
    ...
    RAA_Smear_measured[i]->Scale(1./145.156);
    ...
    RAA_Smear_binbybin[i]->Scale(1./145.156);
  ...
  //  TFile fout(Form("Pawan_ntuple_PbPb_pp_calopfpt_ppNoJetidcut_R0p%d_noFakeWeightwithout80_unfold_mcclosure_oppside_trgMC_BothSmear_40unf_%s_%dGeVCut_ak%s_%d.root", radius, etaWidth, unfoldingCut, jet_type, date.GetDate()),"RECREATE");
 
  //   TFile fout(Form("Pawan_ntuple_PbPb_pp_calopfpt_ppNoJetidcut_R0p%d_AllMBFakeRemoval_unfold_mcclosure_oppside_trgMC_BothSmear_40unf_%s_%dGeVCut_ak%s_%d.root", radius, etaWidth, unfoldingCut, jet_type, date.GetDate()),"RECREATE");

  TFile fout(Form("Pawan_ntuple_PbPb_pp_calopfpt_ppNoJetidcut_R0p%d_without80FakeRemoval_unfold_mcclosure_oppside_trgMC_BothSmear_40unf_%s_%dGeVCut_ak%s_%d.root", radius, etaWidth, unfoldingCut, jet_type, date.GetDate()),"RECREATE");

// TFile fout(Form("/afs/cern.ch/work/r/rkunnawa/WORK/RAA/CMSSW_5_3_18/src/Output/Pawan_ntuple_PbPb_pp_calopfpt_ppNoJetidcut_R0p%d_noFakeWeight_unfold_mcclosure_oppside_trgMC_BothSmear_%s_%dGeVCut_ak%s_%d.root", radius, etaWidth, unfoldingCut, jet_type, date.GetDate()),"RECREATE");
  fout.cd();
  ...
  for(int i = 0;i<nbins_cent;++i){
    ...
    uPbPb_JEC_Bayes[i]->Scale(1./145.156);
    ...
    uPbPb_Smear_Bayes[i]->Scale(1./145.156);
