...
void RAA_plot(int radius = 3, char *algo = "Pu", char *jet_type = "PF", int unfoldingCut = 60){
  ...
  //if(location=="MIT") 
  fin= TFile::Open(Form("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/PbPb_pp_calopfpt_jetidcut_R0p%d_unfold_mcclosure_oppside_trgMC_n20_eta_p20_%dGeVCut_ak%s_20150415.root",radius,unfoldingCut,jet_type));
  ...
  TFile * fPP_in = TFile::Open(Form("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/Pp_CutEfficiency_YetkinCuts_matched_slantedlinecalopfpt_addingunmatched_exclusionhighertriggers_eMaxSumcand_A_R0p%d.root",radius));
  TFile * fPP_in_trigger = TFile::Open(Form("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/Pp_CutEfficiency_YetkinCuts_matched_slantedlinecalopfpt_addingunmatched_exclusionhighertriggers_eMaxSumcand_A_R0p%d_for_trigger.root",radius));
  fPP_in->Print();
  ...
  */
  ...
  if(makeRootFile){
    ...
    TFile fout(Form("/data/users/belt/JetRAA/MatchNtuples/AnalysisPlots/PbPb_data_ak%s%s_testComb4_cut1_%d.root",algo,jet_type,date.GetDate()),"RECREATE");
    fout.cd();
    for(int i = 0;i<=nbins_cent;i++){
  ...
  if(doSpectra){  
    ...
    cSpectra->SaveAs(Form("Plots/Unfolded_spectra_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doPbPbIterSys){
    ...
    cIterSysPbPb->SaveAs(Form("Plots/PbPb_unfoldng_iteration_systematics_%dGeVCut_ak%s%d%s_%d.pdf",unfoldingCut,algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPPIterSys){
    ...
    cIterSysPP->SaveAs(Form("Plots/PP_unfoldng_iteration_systematics_%dGeVCut_ak%d%s_%d.pdf",unfoldingCut,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doRAA){
    ...
    cRAA->SaveAs(Form("Plots/RAA_%dGeVCut_ak%s%d%s_%d.pdf",unfoldingCut,algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPbPbMCClosure){
    ...
    cPbPbMCclosure->SaveAs(Form("Plots/PbPb_unfoldng_mc_closure_opphalf_%dGeVCut_ak%s%d%s_%d.pdf",unfoldingCut,algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPPMCClosure){
    ...
    cPPMCclosure->SaveAs(Form("Plots/PP_unfoldng_mc_closure_test_%dGeVCut_ak%d%s_%d.pdf",unfoldingCut,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPbPbDatavsMC){
    ...
    cPbPb_data_vs_mc->SaveAs(Form("Plots/PbPb_data_vs_mc_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPPDatavsMC){
    ...
    cPP_data_vs_mc->SaveAs(Form("Plots/PP_data_vs_mc_ak%d%s_%d.pdf",radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPbPbNormRes){
    ...
    cPbPb_NormResMat->SaveAs(Form("Plots/PbPb_normalized_response_matrix_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPPNormRes){
    ...
    drawText(Form("Anti-k_{T} %s Jets R=0.%d",jet_type,radius),0.2,0.23,20);
    drawText("|#eta|<2, |vz|<15",0.6,0.31,20);
    cPP_NormResMat->SaveAs(Form("Plots/PP_normalized_response_matrix_ak%d%s_%d.pdf",radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPbPbsigma){
    ...
    cPbPb_sigma->SaveAs(Form("Plots/PbPb_invariant_cross_section_combined_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doPPsigma){
    ...
    cPP_sigma->SaveAs(Form("Plots/PP_invariant_cross_section_ak%d%s_%d.pdf",radius,jet_type,date.GetDate()),"RECREATE");
  }
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(doGenSpectra){
    ...
    cGenSpectra->SaveAs(Form("Plots/MC_spectra_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doPbPbTrgComb){
    ...
    cDataMerge->SaveAs(Form("Plots/PbPb_data_trigger_merging_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  ...
  if(doPbPb12003TrgComb){
    ...
    cData12003Merge->SaveAs(Form("Plots/PbPb_data_trigger_merging_12003_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doPPTrgComb){
    ...
    cPPMerge->SaveAs(Form("Plots/PP_data_trigger_merging_ak%d%s_%d.pdf",radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doPPTrgContribution){
    ...
    cPPTrgCont->SaveAs(Form("Plots/PP_data_trigger_contribution_ak%d%s_%d.pdf",radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doRandomCone){
	...
	cPbPb_MC_jtpu[j]->SaveAs(Form("Plots/bkg_energy_subtracted_%s_ak%s_%s_%d.pdf",etaWidth[j],algo,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doSupernovaData){
    ...
    cSupernova_data->SaveAs(Form("Plots/PbPb_data_supernova_events_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  }
  ...
  if(doSupernovaMC){
    ...
    cSupernova_mc->SaveAs(Form("Plots/PbPb_mc_supernova_events_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doSupernovaDataMCUltraCentral){
    ...
    cSupernovaDataMC->SaveAs(Form("Plots/PbPb_DataMC_supernova_ultraCentral_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");    
  ...
  if(doSupernovaContributionData){
    ...
    for(int i = 0;i<=nbins_cent;i++){
      ...
      cpT_njet_gl7[i]->SaveAs(Form("Plots/PbPb_data_spectra_nJetcut_ak%s%d%s_cent%d_%d.pdf",algo,radius,jet_type,i,date.GetDate()),"RECREATE");
  ...
  if(doSupernovaContributionMC){
    ...
    cMC_njet_lg7->SaveAs(Form("Plots/PbPb_mc_spectra_nJetcut_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doCentDatavsMC){
    ...
    cDataMCCent->SaveAs(Form("Plots/PbPb_datavsmc_centrality_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");

    // TFile fout(Form("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/PbPb_DataMC_cent_ratio_%d.root",date.GetDate()),"RECREATE");
    // fout.cd();
    // hCentRatio->Write();
    // fout.Close();
  ...
  if(doVertexDatavsMC){
    ...
    */
    ...
    cDataMCVertex->SaveAs(Form("Plots/PbPb_datavsmc_vertex_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(doPFElectronCheck){
    ...
    TFile * fhistin = TFile::Open("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/RAA_JetID_Data_mc_ElecCutRejectionStudy_with2DplotsSumMaxChNePhCut_Pu3PF_20141216.root");
    TFile * fMCsubidin = TFile::Open("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/RAA_JetID_mc_subid0_ElecCutRejectionStudy_with2DplotsSumMaxChNePhCut_Pu3PF_20141217.root");
  ...
  if(doJetVariablesCheck){
    ...
    TFile * fData = TFile::Open("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/PbPb_marguerite_chMaxjtpt_norawptcut_electronFailureNtuple_file_spectra_histograms_akPuPF_20150205.root");
    TFile * fMC = TFile::Open("/data/users/belt/JetRAA/MatchNtuples/AnalysisRootFiles/PbPb_mc_chMaxjtpt_norawptcut_spectra_akPuPF_20150210.root");
    ...
    cMaxVariables->SaveAs(Form("Plots/PbPb_JetVariables_Max_centFull_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    cSumVariables->SaveAs(Form("Plots/PbPb_JetVariables_Sum_centFull_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    //
    //
    //
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cchMax->SaveAs(Form("Plots/PbPb_JetVariables_chMax_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cchSum->SaveAs(Form("Plots/PbPb_JetVariables_chSum_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cphMax->SaveAs(Form("Plots/PbPb_JetVariables_phMax_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cphSum->SaveAs(Form("Plots/PbPb_JetVariables_phSum_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cneMax->SaveAs(Form("Plots/PbPb_JetVariables_neMax_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cneSum->SaveAs(Form("Plots/PbPb_JetVariables_neSum_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cmuMax->SaveAs(Form("Plots/PbPb_JetVariables_muMax_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    cmuSum->SaveAs(Form("Plots/PbPb_JetVariables_muSum_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
   ...
   // make plot for eMax
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    ceMax->SaveAs(Form("Plots/PbPb_JetVariables_eMax_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.5,0.33,16);
    drawText("pCES, HBHE(data)",0.5,0.43,16);
    ceSum->SaveAs(Form("Plots/PbPb_JetVariables_eSum_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    drawText("|#eta|<2, |vz|<15",0.3,0.33,16);
    drawText("pCES, HBHE",0.3,0.43,16);
    cjtpu->SaveAs(Form("Plots/PbPb_JetVariables_jtpu_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    */
  ...
  if(doJetID){
    ...
    cCut1->SaveAs(Form("Plots/PbPb_JetID_cut1_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    cCut2->SaveAs(Form("Plots/PbPb_JetID_cut2_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    cCut3->SaveAs(Form("Plots/PbPb_JetID_cut3_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    cCut4->SaveAs(Form("Plots/PbPb_JetID_cut4_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    ...
    cCut5->SaveAs(Form("Plots/PbPb_JetID_cut5_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
  ...
  if(do2DCutpTplot){
    ...
    if(do2DCut1){
      ...
      cCut1_pt->SaveAs(Form("Plots/PbPb_JetID_cut1_pt_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    }
    ...
    if(do2DCut2){    
      ...
      cCut2_pt->SaveAs(Form("Plots/PbPb_JetID_cut2_pt_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    }
    ...
    if(do2DCut3){
      ...
      cCut3_pt->SaveAs(Form("Plots/PbPb_JetID_cut3_pt_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    }
    ...
    if(do2DCut4){
      ...
      cCut4_pt->SaveAs(Form("Plots/PbPb_JetID_cut4_pt_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    }
    ...
    if(do2DCut5){
      ...
      cCut5_pt->SaveAs(Form("Plots/PbPb_JetID_cut5_pt_ak%s%d%s_%d.pdf",algo,radius,jet_type,date.GetDate()),"RECREATE");
    }
  }
