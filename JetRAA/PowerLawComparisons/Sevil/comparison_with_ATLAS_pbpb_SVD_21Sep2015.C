//run with root -l 'comparison_with_ATLAS_pbpb_SVD_21Sep2015.C(10,60,300,"MEWR")'
// ATLAS: root -l 'comparison_with_ATLAS_pbpb_SVD_21Sep2015.C(10,60,300,"I")'
...
void doCorr(TH1F *h1, TH1F *h2, int mode = 0)
{
  for(int ix = 1; ix<=h2->GetNbinsX(); ++ix){
    ...
    switch (mode){
    case 0:
      cout<<val<<"   case 0 "<<xbin<<endl;
      val = val * h2->GetBinContent(ix);
      cout<<val<<"   case 0 after binning "<<xbin<<endl;
      break;
    case 1:
      val = val * (double)1./h2->GetBinContent(ix);

      cout<<val<<"   case 1 "<<xbin<<endl;
      val = val * h2->GetBinContent(ix);
      cout<<val<<"   case 1 after binning "<<xbin<<endl;
      break;
    case 2:
...
void comparison_with_ATLAS_pbpb_SVD_21Sep2015(Int_t nfit=10, Int_t FitStart=60, Int_t FitEnd=300, Option_t *FitOpt="MEWR")
{
...
//=========  (Wed Sep 16 22:06:36 2015) by ROOT version5.32/00
   Double_t xAxis2412[11] = {30, 40, 50, 60, 80, 100, 120, 160, 200, 250, 320};  //cms bins 
...
//      uPP_R4_ATLASComp->Fit("fitppCMS",FitOpt,"",60,300); //fit function    
  ...
  double par0=fitCMS_periph->GetParameter(0);
  double par0err=fitCMS_periph->GetParError(0);

  double par1=fitCMS_periph->GetParameter(1);
  double par1err=fitCMS_periph->GetParError(1);

  double par2=fitCMS_periph->GetParameter(2);
  double par2err=fitCMS_periph->GetParError(2);


  cout<<"   par 0  "<<par0<<"   "<<par0err<<"   "<<par1<<"   "<<par1err<<"   "<<par2<<"  "<<par2err<<endl;
  TF1 *fitCMS_periph_fix = new TF1("fitCMS_periph_fix","[0]*pow(x+[2],[1])"); //create function
  fitCMS_periph_fix->FixParameter(0,par0+par0err);
  fitCMS_periph_fix->FixParameter(1,par1-par1err);
  fitCMS_periph_fix->FixParameter(2,par2+par2err);
      
  double Apar0=fitATLAS_periph->GetParameter(0);
  double Apar0err=fitATLAS_periph->GetParError(0);

  double Apar1=fitATLAS_periph->GetParameter(1);
  double Apar1err=fitATLAS_periph->GetParError(1);

  double Apar2=fitATLAS_periph->GetParameter(2);
  double Apar2err=fitATLAS_periph->GetParError(2);


   cout<<"   Atlas par0  "<<Apar0<<"   "<<Apar0err<<"   "<<Apar1<<"   "<<Apar1err<<"   "<<Apar2<<"  "<<Apar2err<<endl;
  TF1 *fitATLAS_periph_fix = new TF1("fitATLAS_periph_fix","[0]*pow(x+[2],[1])"); //create function
  fitATLAS_periph_fix->FixParameter(0,Apar0-Apar0err);
  fitATLAS_periph_fix->FixParameter(1,Apar1+Apar1err);
  fitATLAS_periph_fix->FixParameter(2,Apar2-Apar2err);
...
//    cATLAS_HistComp_3->SetFrameBorderMode(0);cd cen0
...
//
   ...
   for(int i=0; i<nfit; ++i){
     uATLAS_PbPb_R4_cent0->Fit("fitATLAS_mostcent","","",FitStart,FitEnd); //fit function
   }    
   // clone before fit so I don't get that stupid line in the ratio plot
...
//    }
    ...
    double A0par0=fitATLAS_mostcent->GetParameter(0);
    double A0par0err=fitATLAS_mostcent->GetParError(0);
    
    double A0par1=fitATLAS_mostcent->GetParameter(1);
    double A0par1err=fitATLAS_mostcent->GetParError(1);
    
    double A0par2=fitATLAS_mostcent->GetParameter(2);
    double A0par2err=fitATLAS_mostcent->GetParError(2);


    //   cout<<"   Atlas par0  "<<Apar0<<"   "<<Apar0err<<"   "<<Apar1<<"   "<<Apar1err<<"   "<<Apar2<<"  "<<Apar2err<<endl;
  TF1 *fitATLAS_mostcent_fix = new TF1("fitATLAS_mostcent_fix","[0]*pow(x+[2],[1])"); //create function
  fitATLAS_mostcent_fix->FixParameter(0,A0par0+A0par0err);
  fitATLAS_mostcent_fix->FixParameter(1,A0par1-A0par1err);
  fitATLAS_mostcent_fix->FixParameter(2,A0par2+A0par2err);
    
  TH1F *hfitCMScent0=new TH1F("fitCMScent0","fitCMScent0",10,xAxis2420);
  hfitCMScent0 = (TH1F*)functionHist(fitCMS_mostcent,hfitCMScent0,"fit_cms_cent0");
  hfitCMScent0->Print("base");
  TH1F* hCMSDataoverFitcent0=(TH1F*)uPbPb_R4_ATLASComp_cent0->Clone("CMSDataoverFitcent0");
  hCMSDataoverFitcent0->Divide(hfitCMScent0); 
  
  TH1F *hfitATLAScent0=new TH1F("fitATLAScent0","fitATLAScent0",12,xAxis2421);
  hfitATLAScent0 = (TH1F*)functionHist(fitATLAS_mostcent,hfitATLAScent0,"fit_atlas_cent0");
  TH1F* hATLASDataoverFitcent0=(TH1F*)uATLAS_PbPb_R4_cent0->Clone("ATLASDataoverFitcent0");
  hATLASDataoverFitcent0->Divide(hfitATLAScent0); 
 
 TF1 *fitATLAS_corr_0 = new TF1("fitATLAS_corr_0","[0]*pow(x,3)+[1]*pow(x,2)+[2]*x+[3]+[4]*pow(x,4)+[5]*pow(x,5)"); //create function
     for(int i=0; i<nfit; ++i){
  hATLASDataoverFitcent0->Fit("fitATLAS_corr_0",FitOpt,"",FitStart,FitEnd);
}


  TH1F *hfitATLAScent1=new TH1F("fitATLAScent1","fitATLAScent1",12,xAxis2421);
  hfitATLAScent1 = (TH1F*)functionHist(fitATLAS_cent,hfitATLAScent1,"fit_atlas_cent1");
  TH1F* hATLASDataoverFitcent1=(TH1F*)uATLAS_PbPb_R4_cent1->Clone("ATLASDataoverFitcent1");
  hATLASDataoverFitcent1->Divide(hfitATLAScent1); 
  TF1 *fitATLAS_corr_1 = new TF1("fitATLAS_corr_1","[0]*pow(x,3)+[1]*pow(x,2)+[2]*x+[3]+[4]*pow(x,4)"); //create function
   for(int i=0; i<nfit; ++i){
  
  hATLASDataoverFitcent1->Fit("fitATLAS_corr_1",FitOpt,"",FitStart,FitEnd);
}

  TH1F *hfitATLAScent3=new TH1F("fitATLAScent3","fitATLAScent3",12,xAxis2421);
  hfitATLAScent3 = (TH1F*)functionHist(fitATLAS_midcent,hfitATLAScent3,"fit_atlas_cent3");
  TH1F* hATLASDataoverFitcent3=(TH1F*)uATLAS_PbPb_R4_cent3->Clone("ATLASDataoverFitcent3");
  hATLASDataoverFitcent3->Divide(hfitATLAScent3); 
  TF1 *fitATLAS_corr_3 = new TF1("fitATLAS_corr_3","[0]*pow(x,3)+[1]*pow(x,2)+[2]*x+[3]+[4]*pow(x,4)"); //create function
    for(int i=0; i<nfit; ++i){

  hATLASDataoverFitcent3->Fit("fitATLAS_corr_3",FitOpt,"",FitStart,FitEnd);
}
  
 
  TH1F *hfitATLAScent5=new TH1F("fitATLAScent5","fitATLAScent5",12,xAxis2421);
  hfitATLAScent5 = (TH1F*)functionHist(fitATLAS_midperiph,hfitATLAScent5,"fit_atlas_cent5");
  TH1F* hATLASDataoverFitcent5=(TH1F*)uATLAS_PbPb_R4_cent5->Clone("ATLASDataoverFitcent5");
  hATLASDataoverFitcent5->Divide(hfitATLAScent5); 
  TF1 *fitATLAS_corr_5 = new TF1("fitATLAS_corr_5","[0]*pow(x,3)+[1]*pow(x,2)+[2]*x+[3]+[4]*pow(x,4)+[6]*pow(x,6)+[5]*pow(x,5)"); //create function
    for(int i=0; i<nfit; ++i){

  hATLASDataoverFitcent5->Fit("fitATLAS_corr_5",FitOpt,"",FitStart,FitEnd);
}
  
  

  TH1F *hfitATLAScent7=new TH1F("fitATLAScent7","fitATLAScent7",12,xAxis2421);
  hfitATLAScent7 = (TH1F*)functionHist(fitATLAS_periph,hfitATLAScent7,"fit_atlas_cent7");
  TH1F* hATLASDataoverFitcent7=(TH1F*)uATLAS_PbPb_R4_cent7->Clone("ATLASDataoverFitcent7");
  hATLASDataoverFitcent7->Divide(hfitATLAScent7); 
  TF1 *fitATLAS_corr_7 = new TF1("fitATLAS_corr_7","[0]*pow(x,3)+[1]*pow(x,2)+[2]*x+[3]+[4]*pow(x,4)"); //create function
    for(int i=0; i<nfit; ++i){
  hATLASDataoverFitcent7->Fit("fitATLAS_corr_7",FitOpt,"",FitStart,FitEnd);
}


  
  // doCorr(hRatioCMS_periph_axes,hCMSDataoverFitcent0,0);//mode 0 is multiplication
    // doCorr(hRatioCMS_periph_axes,hATLASDataoverFitcent0,1); //mode 1 is dividing
...
// ------------>Primitives in pad: cATLAS_HistComp_2 PERIPHERAL
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("uATLAS_PbPb_R4_cent7","ATLAS PbPb 70-80%, R=0.4, fit","pl");
...
// ------------>Primitives in pad: cATLAS_HistComp_3 50-60 or 50-70%
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("uATLAS_PbPb_R4_cent5","ATLAS PbPb 50-60%, R=0.4, fit","pl");
...
// ------------>Primitives in pad: cATLAS_HistComp_4
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("uATLAS_PbPb_R4_cent3","ATLAS PbPb 30-40%, R=0.4, fit","pl");
...
// ------------>Primitives in pad: cATLAS_HistComp_5 10-20 or 10-30%
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("uATLAS_PbPb_R4_cent1","ATLAS PbPb 10-20%, R=0.4, fit","pl");
...
// ------------>Primitives in pad: cATLAS_HistComp_6 0-5 or 0-10%
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("uATLAS_PbPb_R4_cent0","ATLAS PbPb 0-10%, R=0.4, fit","pl");
   ...
   cATLAS_HistComp->SaveAs("Plots/ATLAScorrpowerlawfit_CMS_SVD_PbPbSpectra.pdf");
   
// Draw correction factor histos:
// DRAW CANVAS STUFF
   TCanvas *cATLAS_CorrComp = new TCanvas("cATLAS_CorrComp", "",0,0,1400,1000);
   gStyle->SetOptStat(0);
   cATLAS_CorrComp->Range(0,0,1,1);
   cATLAS_CorrComp->SetFillColor(0);
   cATLAS_CorrComp->SetBorderMode(0);
   cATLAS_CorrComp->SetBorderSize(2);
   cATLAS_CorrComp->SetFrameBorderMode(0);
   TH1F *hBlank = new TH1F("hBlank"," ",100,60,300);
   hBlank->SetMinimum(0.);
   hBlank->SetMaximum(2.);
   hBlank->SetDirectory(0);
   hBlank->SetStats(0);
   hBlank->SetFillColor(1);
   hBlank->SetFillStyle(0);
   hBlank->SetLineStyle(0);
   hBlank->SetMarkerStyle(20);
   hBlank->SetMarkerSize(1.2);
   hBlank->GetXaxis()->SetTitle("ak R=0.4 Jet p_{T} (GeV/c)");
   hBlank->GetXaxis()->SetLabelFont(42);
   hBlank->GetXaxis()->SetLabelOffset(0.01);
   hBlank->GetXaxis()->SetLabelSize(0.045);
   hBlank->GetXaxis()->SetTitleSize(0.055);
   hBlank->GetXaxis()->SetTitleFont(42);
   hBlank->GetYaxis()->SetTitle("ATLAS fit correction");
   hBlank->GetYaxis()->SetLabelFont(42);
   hBlank->GetYaxis()->SetLabelOffset(0.01);
   hBlank->GetYaxis()->SetLabelSize(0.045);
   hBlank->GetYaxis()->SetTitleSize(0.055);
   hBlank->GetYaxis()->SetTitleOffset(1.5);
   hBlank->GetYaxis()->SetTitleFont(42);
   hBlank->GetZaxis()->SetLabelFont(42);
   hBlank->GetZaxis()->SetLabelSize(0.045);
   hBlank->GetZaxis()->SetTitleSize(0.035);
   hBlank->GetZaxis()->SetTitleFont(42);  
// ------------>Primitives in pad: cATLAS_CorrComp_1 EMPTY
   TPad *cATLAS_CorrComp_1 = new TPad("cATLAS_CorrComp_1", "cATLAS_CorrComp_1",0.01,0.51,0.3233333,0.99);
   cATLAS_CorrComp_1->Draw();
   cATLAS_CorrComp_1->cd();
   cATLAS_CorrComp_1->Range(0,0,1,1);
   cATLAS_CorrComp_1->SetFillColor(0);
   cATLAS_CorrComp_1->SetBorderMode(0);
   cATLAS_CorrComp_1->SetBorderSize(2);
   cATLAS_CorrComp_1->SetFrameBorderMode(0);
   cATLAS_CorrComp_1->Modified();
   cATLAS_CorrComp->cd();
  
// ------------>Primitives in pad: cATLAS_CorrComp_2 PERIPHERAL
   cATLAS_CorrComp_2 = new TPad("cATLAS_CorrComp_2", "cATLAS_CorrComp_2",0.3433333,0.51,0.6566667,0.99);
   cATLAS_CorrComp_2->Draw();
   cATLAS_CorrComp_2->cd();
   cATLAS_CorrComp_2->Range(4.999997,-4.900196,355,2.378303);
   cATLAS_CorrComp_2->SetFillColor(0);
   cATLAS_CorrComp_2->SetBorderMode(0);
   cATLAS_CorrComp_2->SetBorderSize(2);
   cATLAS_CorrComp_2->SetFrameBorderMode(0);
   cATLAS_CorrComp_2->SetFrameBorderMode(0);
    hBlank->Draw();
      hATLASDataoverFitcent7->Draw("same");
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("hATLASDataoverFitcent7","ATLAS PbPb 70-80%, R=0.4, fit","pl");
   ...
   TText *text = pt->AddText(" ");
   pt->Draw();
   cATLAS_CorrComp_2->Modified();
   cATLAS_CorrComp->cd();
      
 // different pad

  
// ------------>Primitives in pad: cATLAS_CorrComp_3 50-60 or 50-70%
   cATLAS_CorrComp_3 = new TPad("cATLAS_CorrComp_3", "cATLAS_CorrComp_3",0.6766667,0.51,0.99,0.99);
   cATLAS_CorrComp_3->Draw();
   cATLAS_CorrComp_3->cd();
   cATLAS_CorrComp_3->Range(4.999997,-4.597483,355,2.289306);
   cATLAS_CorrComp_3->SetFillColor(0);
   cATLAS_CorrComp_3->SetBorderMode(0);
   cATLAS_CorrComp_3->SetBorderSize(2);
   cATLAS_CorrComp_3->SetFrameBorderMode(0);
   cATLAS_CorrComp_3->SetFrameBorderMode(0);
   hBlank->Draw();
   hATLASDataoverFitcent5->Draw("same");
   leg = new TLegend(0.25,0.75,0.5,0.85,NULL,"brNDC");
   leg->SetBorderSize(0);
   leg->SetTextFont(62);
   leg->SetTextSize(0.04);
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("hATLASDataoverFitcent5","ATLAS PbPb 50-60%, R=0.4, fit","pl");
   ...
   text = pt->AddText(" ");
   pt->Draw();
   cATLAS_CorrComp_3->Modified();
   cATLAS_CorrComp->cd();     
// new pad
// ------------>Primitives in pad: cATLAS_CorrComp_4
   cATLAS_CorrComp_4 = new TPad("cATLAS_CorrComp_4", "cATLAS_CorrComp_4",0.01,0.01,0.3233333,0.49);
   cATLAS_CorrComp_4->Draw();
   cATLAS_CorrComp_4->cd();
   cATLAS_CorrComp_4->Range(4.999997,-4.223266,355,2.117955);
   cATLAS_CorrComp_4->SetFillColor(0);
   cATLAS_CorrComp_4->SetBorderMode(0);
   cATLAS_CorrComp_4->SetBorderSize(2);
   cATLAS_CorrComp_4->SetFrameBorderMode(0);
   cATLAS_CorrComp_4->SetFrameBorderMode(0);
   hBlank->Draw();
   hATLASDataoverFitcent3->Draw("same");
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("hATLASDataoverFitcent3","ATLAS PbPb 30-40%, R=0.4, fit","pl");
   ...
   text = pt->AddText(" ");
   pt->Draw();
   cATLAS_CorrComp_4->Modified();
   cATLAS_CorrComp->cd();
// new pad
// ------------>Primitives in pad: cATLAS_CorrComp_5 10-20 or 10-30%
   cATLAS_CorrComp_5 = new TPad("cATLAS_CorrComp_5", "cATLAS_CorrComp_5",0.3433333,0.01,0.6566667,0.49);
   cATLAS_CorrComp_5->Draw();
   cATLAS_CorrComp_5->cd();
   cATLAS_CorrComp_5->Range(4.999997,-4.283988,355,2.105039);
   cATLAS_CorrComp_5->SetFillColor(0);
   cATLAS_CorrComp_5->SetBorderMode(0);
   cATLAS_CorrComp_5->SetBorderSize(2);
   cATLAS_CorrComp_5->SetFrameBorderMode(0);
   cATLAS_CorrComp_5->SetFrameBorderMode(0);
   hBlank->Draw();
  hATLASDataoverFitcent1->Draw("same");
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("hATLASDataoverFitcent1","ATLAS PbPb 10-20%, R=0.4, fit","pl");
   ...
   text = pt->AddText(" ");
   pt->Draw();
   cATLAS_CorrComp_5->Modified();
   cATLAS_CorrComp->cd();
// new pad
// ------------>Primitives in pad: cATLAS_CorrComp_6 0-5 or 0-10%
   cATLAS_CorrComp_6 = new TPad("cATLAS_CorrComp_6", "cATLAS_CorrComp_6",0.6766667,0.01,0.99,0.49);
   cATLAS_CorrComp_6->Draw();
   cATLAS_CorrComp_6->cd();
   cATLAS_CorrComp_6->Range(4.999997,-4.385961,355,2.496327);
   cATLAS_CorrComp_6->SetFillColor(0);
   cATLAS_CorrComp_6->SetBorderMode(0);
   cATLAS_CorrComp_6->SetBorderSize(2);
   cATLAS_CorrComp_6->SetFrameBorderMode(0);
   cATLAS_CorrComp_6->SetFrameBorderMode(0);
  hBlank->Draw();
   hATLASDataoverFitcent0->Draw("same");
   ...
   entry->SetMarkerStyle(27);
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("hATLASDataoverFitcent0","ATLAS PbPb 0-10%, R=0.4, fit","pl");
   ...
   text = pt->AddText(" ");
   pt->Draw();
   cATLAS_CorrComp_6->Modified();
   cATLAS_CorrComp->cd();
   cATLAS_CorrComp->Modified();
   cATLAS_CorrComp->cd();
   cATLAS_CorrComp->SetSelected(cATLAS_CorrComp);
   cATLAS_CorrComp->SaveAs("Plots/ATLAScorrpowerlawFactor_SVD_PbPbSpectra.pdf");   
...
//   TH1F *hFitRatioCMS_periph = (TH1F*)functionHist(fitCMS_periph,hRatioCMS_periph_axes,"hFitRatioCMS_periph"); //make histo hFitRatioCMS from fitppCMS function with hRatioCMS_axes bins
   ...
   /* TH1F *hfitCMScent5=new TH1F("fitCMScent5","fitCMScent5",10,xAxis2412);
   hfitCMScent5 = (TH1F*)functionHist(fitCMS_periph,hfitCMScent5,"fit_cms_cent5");
   hfitCMScent5->Print("base");
   */
   
   TH1F* hFitRatioCMS_periph=(TH1F*)uPbPb_R4_ATLASComp_cent5->Clone("hFitRatioCMS_periph");
   //hCMSDataoverFitcent5->Divide(hfitCMScent5); 
    
   /*TH1F *hfitATLAScent7=new TH1F("fitATLAScent7","fitATLAScent7",12,xAxis2413);
   hfitATLAScent7 = (TH1F*)functionHist(fitATLAS_periph,hfitATLAScent7,"fit_atlas_cent7");
   hfitATLAScent7->Print("base");
   TH1F* hATLASDataoverFitcent7=(TH1F*)uATLAS_PbPb_R4_cent7->Clone("ATLASDataoverFitcent7");
   hATLASDataoverFitcent7->Divide(hfitATLAScent7);
   */
   TH1F *hFitBottomATLAScorr_periph = (TH1F*)functionHist(fitATLAS_corr_7,hRatioCMS_periph_axes,"hFitBottomATLAScorr_periph"); // make histo hFitBottomATLAS from fitppATLAS with hRatioCMS_axes bins
  
   //  doCorr(hFitRatioCMS_periph,hCMSDataoverFitcent5,0);//mode 0 is multiplication
   //  doCorr(hRatioCMS_periph_axes,hATLASDataoverFitcent7,1); //mode 1 is dividing
 
   hFitRatioCMS_periph->Divide(hFitBottomATLAS_periph);
    hFitRatioCMS_periph->Divide(hFitBottomATLAScorr_periph);
   ...
   TH1F* hFitRatioCMS_midperiph=(TH1F*)uPbPb_R4_ATLASComp_cent4->Clone("hFitRatioCMS_midperiph");
 ...
 TH1F *hFitBottomATLAScorr_midperiph = (TH1F*)functionHist(fitATLAS_corr_5,hRatioCMS_midperiph_axes,"hFitBottomATLAScorr_midperiph"); // make histo hFitBottomATLAS from fitppATLAS with hRatioCMS_axes b

 hFitRatioCMS_midperiph->Divide(hFitBottomATLAS_midperiph);
   hFitRatioCMS_midperiph->Divide(hFitBottomATLAScorr_midperiph);
   ...
   TH1F* hFitRatioCMS_midcent=(TH1F*)uPbPb_R4_ATLASComp_cent3->Clone("hFitRatioCMS_midcent");
   ...
   TH1F *hFitBottomATLAScorr_midcent = (TH1F*)functionHist(fitATLAS_corr_3,hRatioCMS_midcent_axes,"hFitBottomATLAScorr_midcent"); // make histo hFitBottomATLAS from fitppATLAS with hRatioCMS_axes b
   
   hFitRatioCMS_midcent->Divide(hFitBottomATLAS_midcent);
   hFitRatioCMS_midcent->Divide(hFitBottomATLAScorr_midcent);
   ...
   TH1F* hFitRatioCMS_cent=(TH1F*)uPbPb_R4_ATLASComp_cent2->Clone("hFitRatioCMS_cent");
   //  fitATLAS_cent->Divide(fitATLAS_corr_0);
 ...
 TH1F *hFitBottomATLAScorr_cent = (TH1F*)functionHist(fitATLAS_corr_1,hRatioCMS_cent_axes,"hFitBottomATLAS_cent"); // make histo hFitBottomATLAS from fitppATLAS with hRatioCMS_axes bins

   
   hFitRatioCMS_cent->Divide(hFitBottomATLAS_cent);
       hFitRatioCMS_cent->Divide(hFitBottomATLAScorr_cent);
   ...
   TH1F *hFitBottomATLAScorr_mostcent = (TH1F*)functionHist(fitATLAS_corr_0,hRatioCMS_mostcent_axes,"hFitBottomATLAScorr_mostcent"); // make histo hFitBottomATLAS from fitppATLAS with hRatioCMS_axes bins
  
   
   TH1F* hFitRatioCMS_mostcent=(TH1F*)uPbPb_R4_ATLASComp_cent0->Clone("hFitRatioCMS_mostcent");
   hFitRatioCMS_mostcent->Divide(hFitBottomATLAS_mostcent);
   
   hFitRatioCMS_mostcent->Divide(hFitBottomATLAScorr_mostcent);
   ...
   TH1F *hRatioBlank = new TH1F("hRatioBlank"," ",100,60,300);
...
//    box->Draw();
   ...
   leg->SetFillColor(19);
   leg->SetFillStyle(0);
   TLegendEntry *entry=leg->AddEntry("hFitRatioCMS_periph","CMS 70-90%/ATLAS 70-80%, R=0.4 fit","pl");
   leg->Draw();
...
// and for PbPb, lets just point the ratio points till 180 for 70-90% and 240 for 50-70%   
   ...
   leg->SetFillColor(19);
   leg->SetFillStyle(0);
   entry=leg->AddEntry("hFitRatioCMS_midperiph","CMS 50-70%/ATLAS 50-60%, R=0.4 fit","pl");
   leg->Draw();
...
// ------------>Primitives in pad: cATLAS_Ratio_4
   ...
   leg->SetFillColor(19);
   leg->SetFillStyle(0);
   entry=leg->AddEntry("hFitRatioCMS_midcent","CMS 30-50%/ATLAS 30-40%, R=0.4 fit","pl");
   leg->Draw();
...
// ------------>Primitives in pad: cATLAS_Ratio_5 10-20 or 10-30%
   ...
   leg->SetFillColor(19);
   leg->SetFillStyle(0);
   entry=leg->AddEntry("hFitRatioCMS_cent","CMS 10-30%/ATLAS 10-20%, R=0.4 fit","pl");
   leg->Draw();
...
// ------------>Primitives in pad: cATLAS_Ratio_6 0-5 or 0-10%
   ...
   leg->SetFillColor(19);
   leg->SetFillStyle(0);
   entry=leg->AddEntry("hFitRatioCMS_mostcent","CMS 0-5%/ATLAS 0-10%, R=0.4 fit","pl");
   leg->Draw();
...
line->Draw();
   ...
   cATLAS_Ratio->SaveAs("Plots/ATLAScorrpowerlawfit_CMS_SVD_Ratio_PbPbSpectra.pdf");           
}
